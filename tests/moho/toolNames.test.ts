...
describe('toolNames: карта переименования Moho-тулов', () => {
  ...
  describe('renamed()', () => {
    ...
    /**
     * НАЙДЕННЫЙ ДЕФЕКТ (владелец файла — координатор, здесь только фиксируется).
     *
     * `renamed()` ищет имя через `MOHO_TOOL_NAMES[oldName]`, то есть по всей
     * цепочке прототипов. Для `toString`, `constructor`, `valueOf`,
     * `hasOwnProperty` возвращается унаследованная от `Object.prototype`
     * ФУНКЦИЯ; проверка `if (!next)` её не отсекает, потому что функция
     * истинна. Вместо честного отказа наружу уходит нативная функция.
     *
     * Практический риск сейчас нулевой: имена тулов в tools.ts никогда не
     * совпадают с членами Object.prototype, что и закреплено тестом ниже.
     * Починка — `Object.hasOwn(MOHO_TOOL_NAMES, oldName)` либо
     * `Object.create(null)` как основа карты.
     */
    it('ни один ключ карты не совпадает с членом Object.prototype', () => {
      // Пока это верно, утечка по цепочке прототипов недостижима из рабочего
      // пути: коллектор зовёт renamed() только с именами из tools.ts.
      const inherited = oldNames.filter(name => !Object.hasOwn(MOHO_TOOL_NAMES, name));
      expect(inherited).toEqual([]);
    });

    it('renamed на членах Object.prototype отдаёт не-строку вместо отказа (задокументированный дефект)', () => {
      // Тест намеренно фиксирует ФАКТИЧЕСКОЕ поведение, а не желаемое: он не
      // мой файл. Когда координатор добавит Object.hasOwn, этот тест упадёт и
      // потребует замены на `expect(() => ...).toThrow()` — то есть дефект не
      // будет закрыт молча.
      const leaked = renamed('toString');
