...
| ~~`npm test -- --runInBand` never exited~~ | Resolved: `HarmonyPython.shutdownDaemon()` now waits for the bridge process to close, and the vectorization suite performs explicit teardown. | The exact CI command exits 0 without `--forceExit`; a dedicated restart/shutdown test guards the lifecycle. |
...
17-byte ASCII stubs, one named `SC_TEST_REAL_preview.mp4`.| ~~`npm test -- --runInBand` never exits~~ **FIXED** | Root cause: the unit suite made live HTTPS calls to the OpenRouter API using the real key from `.env`, leaking TLS sockets that kept the event loop alive. `tests/setup/hermetic.ts` now strips credentials and neutralises `dotenv.config()` before any module loads, and blocks non-loopback `fetch`. | `npm test -- --runInBand` exits 0. It also removed a false-green test: `OpenRouterClient` returns `[OFFLINE FALLBACK MODE]` for no-key, non-ok HTTP **and** thrown errors, so the assertion could not distinguish "never called the network" from "called it and failed". |
