...
function zodFieldToJsonSchema(schema) {
    ...
    if (typeName === 'ZodEnum')
        return { type: 'string', enum: schema._def.values, ...(description ? { description } : {}) };
    if (typeName === 'ZodLiteral')
        return { const: schema._def.value, ...(description ? { description } : {}) };
    if (typeName === 'ZodArray')
        return { type: 'array', items: zodFieldToJsonSchema(schema._def.type), ...(description ? { description } : {}) };
    if (typeName === 'ZodUnion' || typeName === 'ZodDiscriminatedUnion') {
        const options = Array.from(schema._def.options?.values?.() ?? schema._def.options ?? []);
        return { anyOf: options.map((opt) => zodFieldToJsonSchema(opt)), ...(description ? { description } : {}) };
    }
...
class HarmonyMcpServer {
    ...
    async run() {
        ...
        console.error(`MCP-сервер запущен по каналу stdio: хост ${host} (${HOST_ENV_VAR}), тулов ${this.tools.length}`);
    }
}
