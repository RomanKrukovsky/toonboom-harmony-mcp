...
export class RealSceneExecutor {
    ...
    static MINIMAL_PNG_HEX = '89504E470D0A1A0A0000000D49484452000000010000000108060000001F15C4890000000A49444154789C63000100000500010D0A2DB40000000049454E44AE426082';
    ensurePlaceholderAssets(outputDir) {
    ...
    ensureProjectStructure(projectPath, durationFrames = 144, fps = 24) {
...
<project version="3">
  ...
  <scenes>
    ...
    async executeScenePlan(scenePlan, options = {}) {
        ...
        if (!isPlaceholder && !fs.existsSync(resolvedBgPath)) {
            ...
            fs.writeFileSync(path.join(reportsDir, `${sceneName}_execution_report.json`), JSON.stringify(res, null, 2), 'utf8');
            return res;
        }
        ...
        try {
            ...
            if (charPreset === 'left')
                posX = -4.0;
            else if (charPreset === 'right')
                posX = 4.0;
            else if (charPreset === 'foreground_left') {
                posX = -5.0;
                posY = -1.0;
                targetScale = charScale * 1.5;
            }
            else if (charPreset === 'foreground_right') {
                posX = 5.0;
                posY = -1.0;
                targetScale = charScale * 1.5;
            }
            else if (charPreset === 'background_center') {
                posX = 0.0;
                posY = 2.0;
                targetScale = charScale * 0.5;
            }
            try {
            ...
            try {
                ...
                if (camMove === 'slow_push_in') {
                    ...
                    keyframesCreated.push({ node: 'Camera', frame: 1, attr: 'position.z', value: 12 }, { node: 'Camera', frame: duration, attr: 'position.z', value: 10 });
                }
                    ...
                    keyframesCreated.push({ node: 'Camera', frame: 1, attr: 'position.z', value: 10 }, { node: 'Camera', frame: duration, attr: 'position.z', value: 12 });
                }
