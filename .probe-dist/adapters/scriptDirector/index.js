import { directorPlanSchema, directorVariantSetSchema } from '../../schemas/sceneIntelligence.js';
/**
...
export class ScriptDirector {
    ...
    generateVariants(scene, count = 3, requested, seed) {
    ...
    generate(scene, strategy, opts) {
        ...
        const attention = [];
        const pushInBeatIds = [];
        const reactionShotIds = [];
        const emphasizedBeatIds = [];
...
function pickClimaxBeat(beats) {
    if (beats.length === 0)
        return undefined;
    return beats.reduce((top, b) => (b.importance > top.importance ? b : top), beats[0]);
}
function cutTypeFor(profile, beat) {
    if (profile.editingPace === 'fast')
        return beat.intent === 'reveal' ? 'smash' : 'hard';
    if (profile.editingPace === 'medium')
        return 'match_action';
    return 'L_cut';
}
function reactionFraming(profile) {
    if (profile.strategy === 'dramatic_closeup')
        return 'extreme_close_up';
    if (profile.strategy === 'restrained_dialogue')
        return 'medium';
    return 'medium_close';
}
function beatStaging(beat, scene, blocking) {
...
function reactorStaging(beat, scene, blocking) {
...
function eyelineFor(beat, scene) {
    if (scene.characters.length === 1)
        return 'camera';
    return beat.reactionTarget ?? 'offscreen_right';
}
function eyelineForReaction(beat, reactorId, scene) {
    return beat.primaryCharacter;
}
function generatePlanId(sceneId, strategy) {
...
function hashSeed(scene, strategy) {
    let s = 0;
    for (const ch of scene.sceneId + strategy)
