...
import { reconstructionManifestSchema, harmonyCommandPlanSchema } from '../schemas/reconstruction.js';
export class HarmonySceneCompiler {
    comparisonClient;
    constructor(comparisonClient = new ReconstructionClient()) {
        this.comparisonClient = comparisonClient;
    }
    async compile(rawManifest, options) {
        ...
        if (options.dryRun)
            return this.dryRunReport(manifest, target, output);
        if (!fs.existsSync(target))
            throw new HarmonyError('SCENE_NOT_FOUND', `Сцена не найдена: ${target}`);
    ...
    generateCommandPlan(manifest) {
        ...
        const drawingNamesById = new Map();
    ...
    dryRunReport(manifest, target, output) {
        return {
            status: 'validated',
            execution: 'dry_run',
    ...
    prepareTransactionalCopy(source, output, manifestId) {
        const outputDir = path.dirname(output);
        ...
        for (const folder of ['elements', 'palette-library', 'audio']) {
            ...
            if (fs.existsSync(from))
                fs.cpSync(from, path.join(outputDir, folder), { recursive: true, errorOnExist: true });
        }
    ...
    sourceDrawingForFrame(manifest, frame) {
