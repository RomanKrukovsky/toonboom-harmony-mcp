...
export class RecoveryAdapter {
    static async attemptRecovery(stepId, strategy, errorMsg, params) {
        ...
        switch (strategy) {
            ...
            case 'human_confirm': {
                ...
                await WebhookNotifier.sendNotification('Human Intervention Required', `Шаг "${stepId}" заблокирован и требует проверки человеком.\nИнструкция: ${prompt}\nОшибка: ${errorMsg}`, 'warning').catch(() => { });
                return {
                    recovered: false,
            ...
            case 'auto_bailout': {
                ...
                await WebhookNotifier.sendNotification('Automatic Bailout Activated', `Критический сбой на шаге "${stepId}". Запускается сохранение и экстренный выход.\nОшибка: ${errorMsg}`, 'error').catch(() => { });
                try {
