...
export class TemplateAssemblyAdapter {
    ...
    async listTemplates() {
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (stat.isDirectory() || file.endsWith('.tpl') || file.endsWith('.json')) {
                    let type = 'scene';
                    if (file.includes('rig'))
                        type = 'rig';
                    else if (file.includes('camera'))
                        type = 'camera';
                    else if (file.includes('fx'))
                        type = 'fx';
                    else if (file.includes('mouth'))
                        type = 'mouth_chart';
        ...
        if (list.length === 0) {
            list.push({ name: 'default_scene_template', type: 'scene', path: path.join(this.templatesDir, 'default_scene_template.xstage'), description: 'Стандартный шаблон сцены 1080p 24fps' }, { name: 'scientist_rig', type: 'rig', path: path.join(this.templatesDir, 'scientist.tpl'), description: 'Ассет-риг персонажа Scientist' }, { name: 'slow_push_in', type: 'camera', path: path.join(this.templatesDir, 'slow_push_in.json'), description: 'Камера: наезд в течение 192 кадров' }, { name: 'portal_glow_fx', type: 'fx', path: path.join(this.templatesDir, 'portal_glow.json'), description: 'Эффект свечения портала' }, { name: 'standard_mouth_chart', type: 'mouth_chart', path: path.join(this.templatesDir, 'standard_mouth_chart.json'), description: 'Таблица липсинга (A, B, C, D, E, F, G, X)' });
        }
        return list;
    }
    ...
    async createSceneFromTemplate(templatePath, targetPath, options = {}) {
    ...
    async importCharacterRig(projectPath, rigPath, characterName) {
    ...
    async importCameraPreset(projectPath, presetName) {
        return {
            status: 'success',
            preset: presetName,
    ...
    async importFXPreset(projectPath, presetType, targetNode) {
        return {
            status: 'success',
            effect: presetType,
            target: targetNode,
    ...
    async applyMouthChart(projectPath, mouthChartName, lipsyncData) {
        return {
            status: 'success',
    ...
    async applyRenderPreset(projectPath, presetName) {
        return {
            status: 'success',
            preset: presetName,
    ...
    async createTemplatePack(packName) {
        this.ensureTemplatesDir();
