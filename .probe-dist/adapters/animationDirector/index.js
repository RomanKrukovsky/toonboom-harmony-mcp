...
export class AnimationDirector {
    generateCameraPlans(episodePlan, shots) {
        const plans = [];
    ...
    elementsFor(fxNotes) {
        if (!fxNotes)
            return [];
        const out = [];
        if (/explos/i.test(fxNotes))
            out.push('explosion flash', 'smoke');
        if (/portal|wormhole/i.test(fxNotes))
            out.push('portal vortex', 'glow rings');
        if (/particl/i.test(fxNotes))
            out.push('dust motes', 'sparkles');
        if (/smoke/i.test(fxNotes))
            out.push('smoke plume');
        if (out.length === 0)
            out.push('ambient glow');
        return out;
    }
}
