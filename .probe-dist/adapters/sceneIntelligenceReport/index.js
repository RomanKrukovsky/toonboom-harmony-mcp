...
function emotionCurve(scene) {
    if (!scene.emotionCurve.length)
        return '';
...
function attention(scene) {
    if (!scene.attentionTargets.length)
        return '';
...
function continuity(scene) {
    if (!scene.continuity.length)
        return '';
...
function variantBlock(plan, index) {
