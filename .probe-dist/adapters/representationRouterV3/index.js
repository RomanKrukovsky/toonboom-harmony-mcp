import { routingPlanSchema, routingDecisionSchema, representationSegmentSchema, REPRESENTATION_ROUTER_SCHEMA_VERSION } from '../../schemas/representationRouter.js';
function computeFactors(part) {
    ...
    if (states.length === 0) {
        return {
            ...
            estimatedKeyCount: 0, nodeViewComplexity: 'low',
    ...
    const nodeViewComplexity = articulation > 0.6 ? 'high' : articulation > 0.3 ? 'medium' : 'low';
...
function selectRepresentation(part, factors, studioProfile) {
...
function buildAlternatives(part, chosen, factors) {
    const allTypes = [
...
function computeAltScore(rep, factors) {
    switch (rep) {
...
export class RepresentationRouterV3 {
    route(input) {
        ...
        for (const part of decomposition.parts) {
            ...
            if (!segmentMap.has(part.partId))
                segmentMap.set(part.partId, []);
            segmentMap.get(part.partId).push(decision);
        }
        const segments = Array.from(segmentMap.entries()).map(([partId, decs]) => representationSegmentSchema.parse({
            partId,
            segments: decs.map(d => ({
