...
import { digitalActorSchema, DIGITAL_ACTOR_SCHEMA_VERSION } from '../../schemas/digitalActor.js';
...
export class DigitalActorRegistry {
    ...
    validate(actor) {
        ...
        parsedActor.masterDrawings.forEach(d => { if (d.inferred)
            inferredCount++; });
        parsedActor.pivots.forEach(p => { if (p.inferred)
            inferredCount++; });
        parsedActor.poseFamilies.forEach(pf => { if (pf.inferred)
            inferredCount++; });
     ...
     */
    importFromReconstructionManifest(manifestPath, name) {
     ...
     */
    importFromFile(sourceType, sourcePath, name) {
