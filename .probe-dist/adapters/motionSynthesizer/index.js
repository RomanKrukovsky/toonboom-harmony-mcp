import { motionSynthesisPlanSchema } from '../../schemas/keyPoseMotion.js';
export class MotionSynthesizer {
    /**
     * Synthesizes motion tracks by interpolating between key poses.
     */
    synthesize(scene, poses, actor, tolerance = 0.05) {
        const endFrame = scene.endFrame;
        const tracks = [];
        const drawingSubstitutions = [];
        ...
        const properties = ['positionX', 'positionY', 'rotation', 'scaleX', 'scaleY', 'skew'];
     ...
     */
    getInterpolatedValue(keys, frame) {
        const idx = keys.findIndex(k => k.frame >= frame);
        if (idx === -1)
            return keys[keys.length - 1].value;
        if (keys[idx].frame === frame)
