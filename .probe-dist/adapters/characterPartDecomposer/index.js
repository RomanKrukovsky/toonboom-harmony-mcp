import { partDecompositionSchema, partTrackSchema, occlusionEdgeSchema, PART_DECOMPOSITION_SCHEMA_VERSION } from '../../schemas/partDecomposition.js';
...
function buildHumanoidParts(input) {
...
function inferMotionCluster(partId) {
    ...
    if (rigidParts.includes(partId))
        return 'rigid';
    if (articulatedParts.includes(partId))
        return 'articulated';
    if (deformableParts.includes(partId))
        return 'deformable';
    if (staticParts.includes(partId))
        return 'static';
    return 'unknown';
}
function estimateCenterX(partId) {
    const map = {
...
function estimateCenterY(partId) {
    const map = {
...
function detectProblemRanges(states, partId) {
    const problems = [];
    let occludedStart = -1;
...
function detectArticulationHints(states, partId) {
    const hints = [];
...
function buildOcclusionGraph(parts) {
    const edges = [];
    ...
    for (let i = 0; i < sortedParts.length; i++) {
        for (let j = i + 1; j < sortedParts.length; j++) {
            ...
            if (upper.identity.depthOrder <= lower.identity.depthOrder)
                continue;
            let overlapFrames = 0;
            let totalFrames = 0;
