import { cameraLayoutPlanSchema, shotPlanSchema, cameraKeyframeSchema, cameraTrackSchema, blockingPlanSchema, blockingPositionSchema, CAMERA_LAYOUT_SCHEMA_VERSION } from '../../schemas/cameraLayout.js';
/**
 ...
 */
export function determineShotSize(beat, characterCount) {
    ...
    if (characterCount > 2)
        return 'medium_shot';
    if (beat.beatKind === 'pause' || beat.beatKind === 'reaction')
        return 'medium_shot';
    return 'medium_shot';
}
function determineCameraMovement(beat, prevBeat, style) {
    if (style === 'dynamic') {
        if (beat.importance > 0.8)
            return 'dolly_in';
        if (prevBeat && beat.importance > prevBeat.importance)
            return 'pan_right';
        return 'static';
    }
    if (style === 'dramatic') {
        if (beat.importance > 0.85)
            return 'dolly_in';
        if (beat.beatKind === 'revelation')
            return 'dolly_in';
        return 'static';
    }
    if (style === 'comedic') {
        if (beat.emotion === 'surprise' || beat.emotion === 'joy')
            return 'pan_right';
        return 'static';
    }
    if (beat.importance > 0.8)
        return 'dolly_in';
    if (prevBeat && beat.primaryCharacter !== prevBeat.primaryCharacter)
        return 'pan_right';
    return 'static';
}
function calculateCameraPosition(shotSize, focusX, focusY) {
 ...
 */
export function generateFramingRules(shotSize, characterCount) {
    const rules = ['rule_of_thirds', 'headroom'];
    if (characterCount === 1) {
...
function generateShotPlans(input) {
    ...
    for (const beat of beats) {
        const characterCount = characters.filter(c => beat.primaryCharacter === c.characterId ||
            (beat.reactionTarget && beat.reactionTarget === c.characterId)).length || 1;
...
function generateCameraTrack(shots, sceneId, fps) {
    const keyframes = [];
    let frame = 1;
...
function generateBlockingPlans(shots, characters) {
    ...
    generate(input) {
        ...
        return cameraLayoutPlanSchema.parse({
            ...
            summary: {
                ...
                cameraMovements: movementCounts,
                shotSizes: sizeCounts,
