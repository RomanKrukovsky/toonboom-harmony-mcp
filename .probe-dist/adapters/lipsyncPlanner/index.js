...
export class LipsyncPlanner {
    ...
    buildPlan(scene, dialogues, fps) {
        ...
        const missingAssets = [];
        if (!hasAudio)
            missingAssets.push('recorded dialogue audio');
        if (dialogues.length === 0)
            missingAssets.push('final dialogue text');
        return {
    ...
    extractDialogueLines(sceneScript, scene) {
        const lines = [];
    ...
    shapeForChar(char) {
        ...
        if ('pbm'.includes(lower))
            return 'B';
        if ('fv'.includes(lower))
            return 'F';
        if ('ln'.includes(lower))
            return 'G';
        if ('ae'.includes(lower))
            return 'A';
        if ('ou'.includes(lower))
            return 'E';
        if ('i'.includes(lower))
            return 'C';
        if ('r'.includes(lower))
