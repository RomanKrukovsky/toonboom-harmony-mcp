import { commandPlanV3Schema, commandOperationSchema, HARMONY_COMMAND_PLAN_V3_SCHEMA_VERSION } from '../../schemas/harmonyCommandPlanV3.js';
export class HarmonyCommandPlanV3Generator {
    generate(manifest) {
        ...
        if (manifest.digitalActors) {
            for (const actor of manifest.digitalActors) {
                if (actor.characterId)
                    characterGroups.add(actor.characterId);
            }
        }
        ...
        if (manifest.representationSegments) {
            const deformerSegments = manifest.representationSegments.filter(seg => ['curve_deformer', 'envelope_deformer', 'bone_deformer'].includes(seg.representation));
        ...
        if (manifest.criticReports) {
            const humanReviewRequired = manifest.criticReports.some((r) => r.humanReviewRequired);
    ...
    createOperation(operation, order, parameters, description) {
