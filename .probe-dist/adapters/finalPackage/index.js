...
export class FinalPackager {
    assemble(input, outputDir) {
        ...
        const placeholder = input.rig360Specs.some((r) => r.placeholderRigCreated && !r.realRigCreated);
        ...
        if (!fs.existsSync(harmonyProjectDir))
            fs.mkdirSync(harmonyProjectDir, { recursive: true });
        ...
        if (!fs.existsSync(finalRenderDir))
            fs.mkdirSync(finalRenderDir, { recursive: true });
        ...
        const summary = {
            ...
            placeholderRigCount: input.rig360Specs.filter((r) => r.placeholderRigCreated && !r.realRigCreated).length,
            realRigCount: input.rig360Specs.filter((r) => r.realRigCreated).length,
...
- Placeholder Rig Count: ${input.rig360Specs.filter((r) => r.placeholderRigCreated && !r.realRigCreated).length}
...
`;
    ...
    buildRigRequirements(input) {
        return {
            characters: input.characterSpecs.map((c) => ({
                name: c.name,
    ...
    buildRig360Plan(input) {
        return {
            characters: input.rig360Specs.map((r) => ({
                characterName: r.characterName,
    ...
    summarizeWhatWasReal(input) {
        ...
        logs.push({
            ...
            classification: input.characterSpecs.some((c) => c.assetBackend === 'missing') ? 'requires_external_model' : 'generated'
        });
        logs.push({
            module: 'rig360Synthesizer',
            classification: input.rig360Specs.some((r) => !r.realRigCreated && r.placeholderRigCreated) ? 'placeholder' : 'planned'
        });
        ...
        logs.push({
            ...
            classification: (input.lipsyncPlans || []).some((p) => p.origin === 'placeholder') ? 'placeholder' : 'planned'
        });
    ...
    honestSummary(input) {
        const placeholders = input.rig360Specs.filter((r) => r.placeholderRigCreated && !r.realRigCreated).length;
        const lipsyncPlaceholder = (input.lipsyncPlans || []).some((p) => p.origin === 'placeholder');
