import { sceneUnderstandingSchema } from '../../schemas/sceneIntelligence.js';
...
function defaultStance(text) {
    if (/сид|sit/i.test(text))
        return 'sitting';
    if (/леж|lie|lying/i.test(text))
        return 'lying';
    if (/ид|walk|run|бег|иду|движ/i.test(text))
        return 'moving';
    return 'standing';
}
...
function inferEmotion(text, prevLabel) {
...
function inferImportance(text, isLastBeat) {
    let impt = 0.45;
    if (/!$/.test(text.trim()))
        impt += 0.25;
    if (/\?$/.test(text.trim()))
    ...
    if (text.length < 25)
        impt += 0.05;
    if (isLastBeat)
        impt = Math.min(1.0, impt + 0.2);
    return Math.min(1.0, Math.max(0.1, impt));
}
function inferPauseBefore(text, prevText) {
    if (!prevText)
        return 0;
    // pause follows strong emotional beat or question
    if (/[!?.]\s*$/.test(prevText.trim()))
        return 0.35;
    if (/\?\s*$/.test(prevText.trim()))
        return 0.45;
    if (/[""«»]/.test(prevText))
...
function classifyBeatKind(index, count, text) {
...
function splitDialogue(script, dialogue) {
...
export class SceneUnderstandingEngine {
    ...
    analyze(input) {
        ...
        // ── Characters ────────────────────────────────────────────────────────
        const assumedCharacters = new Map();
