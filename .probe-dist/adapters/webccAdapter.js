export class WebccAdapter {
    ...
    static async performAction(actionName, params) {
        if (!this.isAvailable()) {
            return {
                status: 'unsupported',
        ...
        if (!route)
            return { status: 'error', code: 'WEBCC_UNAVAILABLE', message: `Неизвестное действие WebCC: ${actionName}` };
        try {
            ...
            if (!response.ok)
                return { status: 'error', code: 'WEBCC_UNAVAILABLE', message: `WebCC HTTP ${response.status}`, data };
            return {
                status: 'success',
