...
export class HarmonyCli {
    static async render(options) {
        if (!config.harmonyBin) {
            throw new HarmonyError('HARMONY_NOT_INSTALLED', 'Исполняемый файл Harmony Premium не настроен. Задайте HARMONY_BIN.');
        }
        ...
        return new Promise((resolve, reject) => {
            execFile(config.harmonyBin, args, { timeout: config.scriptTimeoutMs * 6 }, (error, stdout, stderr) => {
                if (error) {
                    return reject(new HarmonyError('HARMONY_NOT_INSTALLED', `Сбой CLI рендеринга Harmony: ${error.message}. Stderr: ${stderr}`));
                }
                resolve(stdout);
            });
        });
    }
    static async vectorize(projectPath, drawingPaths) {
        if (!config.harmonyBin) {
            throw new HarmonyError('HARMONY_NOT_INSTALLED', 'Исполняемый файл Harmony Premium не настроен. Задайте HARMONY_BIN.');
        }
        ...
        return new Promise((resolve, reject) => {
            execFile(config.harmonyBin, args, { timeout: config.scriptTimeoutMs * 4 }, (error, stdout, stderr) => {
                if (error) {
                    ...
                    if (error.message.includes('invalid option') || error.message.includes('unknown option')) {
                        return reject(new HarmonyError('UNSUPPORTED_BY_VERSION', `Установленная Harmony не поддерживает пакетную векторизацию через CLI ` +
                            `(-batch -vectorize отклонён). Векторизация возможна через ` +
                            `reconstruction-core или интерфейс Harmony. Детали: ${error.message}`));
                    }
                    return reject(new HarmonyError('VECTORIZE_FAILED', `Сбой CLI векторизации Harmony: ${error.message}`));
                }
                resolve(stdout);
            });
        });
