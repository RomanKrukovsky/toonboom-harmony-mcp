...
export class OnePromptEngine {
    ...
    async generateProductionPackage(input) {
...
async function enhanceScriptWithLlm(script, episodePlan, analysis) {
    ...
    for (const scene of enhanced.scenes) {
        ...
        if (result.status === 'success' && result.dialogue.length > 0) {
            ...
            if (result.origin === 'real')
                enhanced.origin = 'real';
        }
    }
    return enhanced;
}
async function generateLipsyncAudio(lipsyncPlans) {
    return Promise.all(lipsyncPlans.map(async (plan) => {
        if (!plan.dialogues || plan.dialogues.length === 0)
            return plan;
        const enriched = { ...plan, generatedAudio: [], origin: config.backends.audio };
        ...
        if (enriched.generatedAudio.every((a) => a.origin === 'real')) {
            enriched.missingAssets = (enriched.missingAssets || []).filter((m) => m !== 'recorded dialogue audio');
        }
        return enriched;
    }));
}
async function generateBackgroundArt(backgroundPlans) {
...
function emotionForBeat(beat, mood) {
...
function voiceFromBeat(beat, mood) {
...
function voiceForBeat(beat, mood) {
...
function encryptLogLine(prompt) {
    const trimmed = prompt.replace(/\s+/g, ' ').trim();
    return trimmed.length > 200 ? `${trimmed.slice(0, 197)}...` : trimmed;
}
function detectGenre(prompt) {
    const lower = prompt.toLowerCase();
    if (/sci[- ]?fi|науч|космо|робот|иноплан/.test(lower))
        return 'sci-fi comedy';
    if (/фэнтези|магия|дракон|фея/.test(lower))
        return 'fantasy';
    if (/хоррор|жутк|страшн|монстр/.test(lower))
        return 'horror comedy';
    if (/детектив|криминал|расслед/.test(lower))
        return 'detective';
    return 'slice-of-life comedy';
}
function detectTone(prompt) {
    const lower = prompt.toLowerCase();
    if (/драмат|серьёзн|тяжел/.test(lower))
        return 'dramatic';
    if (/дарк|тёмн|кайджи|киберпанк/.test(lower))
        return 'dark';
    return 'comedic';
}
function detectSetting(prompt) {
    const lower = prompt.toLowerCase();
    if (/лаборатор|научн/.test(lower))
        return 'научная лаборатория';
    if (/школ|универс|студент/.test(lower))
        return 'учебное заведение';
    if (/космо|станци|корабл/.test(lower))
        return 'космическая станция';
    if (/город|улиц|квартир/.test(lower))
        return 'городская квартира';
    return 'универсальный сеттинг';
}
function detectAudience(prompt) {
    const lower = prompt.toLowerCase();
    if (/детск|семейн/.test(lower))
        return 'family';
    if (/взросл/.test(lower))
        return 'adult';
    if (/подрост|тинейдж|teen/.test(lower))
        return 'teen';
    return undefined;
}
function parseCandidateCharacters(prompt) {
    const lower = prompt.toLowerCase();
    const candidates = [];
...
function parseThemes(prompt) {
    const themes = [];
    if (/наук|научн/.test(prompt))
        themes.push('ethics of science');
    if (/дружб|friend/.test(prompt))
        themes.push('friendship under pressure');
    if (/комеди|comedy/.test(prompt))
        themes.push('comedic contrast');
    if (themes.length === 0)
