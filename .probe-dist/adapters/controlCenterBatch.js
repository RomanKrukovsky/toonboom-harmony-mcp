...
export class ControlCenterBatch {
    static async runScript(script) {
        if (!config.harmonyCcBin) {
            throw new HarmonyError('CONTROL_CENTER_NOT_FOUND', 'Исполняемый файл Harmony Control Center не найден. Задайте HARMONY_CC_BIN или HARMONY_INSTALL.');
        }
        if (!fs.existsSync(config.harmonyCcBin)) {
            throw new HarmonyError('CONTROL_CENTER_NOT_FOUND', `Исполняемый файл Harmony Control Center отсутствует по пути "${config.harmonyCcBin}".`);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            execFile(config.harmonyCcBin, args, { timeout: config.scriptTimeoutMs }, (error, stdout, stderr) => {
                ...
                if (error) {
                    ...
                    if (isTimeout) {
                        return reject(new HarmonyError('SCRIPT_TIMEOUT', `Пакетная операция Control Center превысила таймаут в ${config.scriptTimeoutMs}мс`));
                    }
                    return reject(new HarmonyError('CONTROL_CENTER_UNREACHABLE', `Пакетное выполнение скрипта Control Center завершилось с ошибкой: ${error.message}. Stderr: ${stderr}`));
                }
                const output = stdout + '\n' + stderr;
                ...
                if (resultMatch) {
                    try {
                        ...
                        return reject(new HarmonyError('CONTROL_CENTER_UNREACHABLE', `Некорректный JSON-ответ от пакетного скрипта Control Center: ${resultMatch[1]}`));
                    }
                }
