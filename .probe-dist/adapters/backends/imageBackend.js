 ...
 */
export async function generateCharacterTurnaround(characterName, view, style, outputPath) {
 ...
 */
export async function generateBackground(location, style, outputPath) {
 ...
 */
async function waitAndDownloadComfyOutput(comfyHost, promptId, destination, timeoutMs = 120_000, pollIntervalMs = 1_000) {
...
async function generateImage(prompt, outputPath, kind) {
    ...
    if (backend === 'comfyui') {
        try {
            ...
            if (promptRes.ok) {
                ...
                throw new Error(`ComfyUI job ${promptId} did not produce a downloadable image within the timeout.`);
            }
        }
        catch (e) {
 ...
 */
export async function vectorizeImageToSVG(imagePath, svgOutputPath) {
    ...
    try {
        ...
        throw new Error(`[SERVICE_UNAVAILABLE] Vectorization requires reconstruction-core on ` +
            `127.0.0.1:${reconPort} (start it with \`npm run reconstruction:core\`). ` +
            `Cause: ${err?.message ?? err}`);
    }
    if (!response?.ok) {
        throw new Error(`[SERVICE_ERROR] reconstruction-core rejected the vectorize request ` +
...
function writePlaceholder(prompt, outputPath, kind) {
