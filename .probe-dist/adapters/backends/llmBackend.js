 ...
 */
export async function generateDialogue(prompt, characters, context) {
...
function buildPrompt(prompt, characters, context) {
    const parts = [
...
function parseJsonDialogue(content) {
    try {
        const cleaned = content.replace(/^```json\s*/, '').replace(/```\s*$/, '');
        const parsed = JSON.parse(cleaned);
        if (Array.isArray(parsed)) {
            return parsed
                .filter((item) => item && typeof item.speaker === 'string' && typeof item.text === 'string')
...
function deterministicDialogue(prompt, characters, context) {
    const lines = [];
