...
export async function captureAppWindow(outputPath) {
    ...
    const { stdout, stderr } = await execFileAsync("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", CAPTURE_SCRIPT], {
        timeout: 15_000,
        env: { ...process.env, [OUTPUT_PATH_ENV]: destination },
    });
    const trimmed = stdout.trim();
    if (!trimmed) {
        throw new Error(`Window capture failed: ${stderr?.trim() || "no output from PowerShell"}`);
    }
