...
import { parseResponse, ErrorCodes, classifyErrno, WireProtocolError, } from "./protocol.js";
...
export class MohoClient {
    ...
    lockHeartbeat = null;
    maxExecutedCache = 2000;
    /** Validate directory safety: non-symlink, owner matches on Unix. */
    async validateDirectorySafety(dirPath) {
        const lstat = await fs.promises.lstat(dirPath);
        if (lstat.isSymbolicLink()) {
            throw new MohoIpcError(`Security Violation: IPC directory ${dirPath} cannot be a symbolic link.`, ErrorCodes.PERMISSION_DENIED, { retryable: false });
        }
        if (os.platform() !== "win32" && typeof process.getuid === "function") {
            const currentUid = process.getuid();
            if (lstat.uid !== currentUid) {
                throw new MohoIpcError(`Security Violation: IPC directory ${dirPath} is owned by UID ${lstat.uid}, expected UID ${currentUid}.`, ErrorCodes.PERMISSION_DENIED, { retryable: false });
            }
        }
    }
    /** Detect legacy /tmp IPC directory and emit a one-time security notice. */
    async detectLegacyTempDir() {
        const legacyTempDir = path.join(os.tmpdir(), "moho-mcp");
        if (path.resolve(config.moho.ipcDir) === path.resolve(legacyTempDir))
            return;
        try {
            const stat = await fs.promises.stat(legacyTempDir);
            if (stat.isDirectory()) {
                process.stderr.write(`[moho-mcp] SECURITY NOTICE: Legacy /tmp IPC directory present at ${legacyTempDir}. ` +
    ...
    async cleanupStaleFiles(ipcDir) {
    ...
    async acquireClientLock(ipcDir) {
        ...
        if (!existing) {
            throw new MohoIpcError(`Cannot acquire IPC client lock at ${lockPath} (held by another live process).`, ErrorCodes.RESOURCE_LOCKED, { retryable: true });
        }
        ...
        if (existing.pid !== -1 && lockAge < config.moho.clientLockTtlMs) {
            throw new MohoIpcError(`Cannot acquire IPC client lock at ${lockPath} (held by PID ${existing.pid}, age ${lockAge}ms).`, ErrorCodes.RESOURCE_LOCKED, { retryable: true, details: { holderPid: existing.pid, ageMs: lockAge } });
        }
        // Stale: steal the lock
        await fs.promises.unlink(lockPath).catch(() => undefined);
        if (!(await tryAcquire())) {
            throw new MohoIpcError(`Cannot acquire IPC client lock at ${lockPath} after stealing stale lock.`, ErrorCodes.RESOURCE_LOCKED, { retryable: true });
        }
        this.startLockHeartbeat(lockPath);
    }
    async readClientLock(lockPath) {
        try {
    ...
    async connect() {
        ...
        this.connected = true;
        startKeepAlive();
        process.stderr.write(`[moho-mcp] Connected to MOHO via secure file IPC at ${ipcDir} (Protocol v${config.server.protocolVersion})\n`);
    }
    ...
    async sendRequest(method, params = {}, options = {}) {
        if (!this.connected) {
            throw new MohoIpcError("Not connected to MOHO. Is the MOHO application running with the MCP plugin loaded?", ErrorCodes.MOHO_NOT_RUNNING, { retryable: true });
        }
        if (this.pendingRequests >= config.moho.maxQueueSize) {
            throw new MohoIpcError(`IPC queue limit exceeded (${this.pendingRequests}/${config.moho.maxQueueSize}). Rejecting request '${method}'.`, ErrorCodes.QUEUE_OVERFLOW, { retryable: true });
        }
        ...
        if (this.executedRequestIds.has(uniqueReqKey)) {
            throw new MohoIpcError(`Duplicate request key rejected: ${uniqueReqKey}`, ErrorCodes.DUPLICATE_REQUEST, { retryable: false });
        }
        ...
        if (this.executedRequestIds.size > this.maxExecutedCache) {
            ...
            if (firstKey !== undefined)
                this.executedRequestIds.delete(firstKey);
        }
        const ipcDir = this.ipcDir;
        ...
        if (Buffer.byteLength(serializedPayload, "utf-8") > maxJsonSizeBytes) {
            throw new MohoIpcError(`JSON request payload exceeds maximum limit of ${maxJsonSizeBytes} bytes.`, ErrorCodes.PAYLOAD_TOO_LARGE, { retryable: false, details: { maxBytes: maxJsonSizeBytes } });
        }
    ...
    async atomicWrite(targetPath, content) {
        ...
        try {
            ...
            if (errno === "ENOSPC" || errno === "EDQUOT") {
                throw new MohoIpcError(`Out of disk space writing ${targetPath}: ${err.message}`, ErrorCodes.PAYLOAD_TOO_LARGE, { retryable: true, details: { errno } });
            }
            throw new MohoIpcError(`Atomic write to ${targetPath} failed: ${err.message}`, ErrorCodes.INTERNAL_ERROR, { retryable: true, details: { errno } });
        }
    }
    /**
     ...
     */
    async isRequestClaimed(reqPath) {
        try {
    ...
    async awaitResponse(respPath, reqPath, method, id, correlationId, options) {
        ...
        while (true) {
            if (options.signal?.aborted) {
                ...
                throw new MohoIpcError(`Request ${method} (id=${id}) cancelled${reason instanceof Error ? `: ${reason.message}` : ""}.`, ErrorCodes.REQUEST_CANCELLED, { retryable: false, correlationId, details: { id, method } });
            }
            try {
                ...
                if (response.id !== null && response.id !== undefined && response.id !== id) {
                    throw new MohoIpcError(`Response id mismatch for ${method}: expected ${id}, received ${String(response.id)}. ` +
                        "Stale response from a previous session — clear the IPC directory.", ErrorCodes.PROTOCOL_MISMATCH, { retryable: false, correlationId, details: { expectedId: id, receivedId: response.id } });
                }
                if (response.error) {
                    throw new MohoIpcError(`MOHO error [${response.error.code}]: ${response.error.message}`, response.error.code, {
                        retryable: false,
                        correlationId,
                ...
                if (err instanceof WireProtocolError) {
                    await fs.promises.unlink(reqPath).catch(() => undefined);
                    throw new MohoIpcError(`Malformed response for ${method} (id=${id}): ${err.message}`, err.code, { retryable: false, correlationId, details: err.details });
                }
                const errno = err.code;
                if (errno === "ENOENT") {
                    ...
                    if (remaining <= 0) {
                        ...
                        if (!claimed)
                            await fs.promises.unlink(reqPath).catch(() => undefined);
                        throw new MohoIpcError(`Request ${method} (id=${id}, corrId=${correlationId}) timed out after ${options.timeout}ms. ` +
                            (claimed
                                ? "Moho claimed the request but never answered — it may be hung on the GUI thread " +
                                    "or showing a modal dialog."
                                : "Moho never picked the request up — the MCP plugin may not be loaded or is not polling."), ErrorCodes.IPC_TIMEOUT, {
                            retryable: true,
                            correlationId,
                    ...
                    // into a 5s freeze.
                    await new Promise((resolve) => setTimeout(resolve, Math.min(options.pollInterval, remaining)));
                    continue;
                }
                throw new MohoIpcError(`IPC read error for ${method}: ${err.message}`, classifyErrno(errno), { retryable: true, correlationId, details: { errno, method, id } });
            }
        }
    }
}
