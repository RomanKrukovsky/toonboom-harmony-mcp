...
export async function startKeepAlive() {
    if (keepAliveProcess || starting)
        return;
    if (!keepAliveEnabled())
        return;
    const platform = os.platform();
    if (platform !== "win32" && platform !== "darwin") {
        process.stderr.write(`[moho-mcp] Keep-alive requested but unsupported on platform '${platform}'; skipping.\n`);
        return;
    }
    starting = true;
    ...
    try {
        ...
        installStopHandler();
        process.stderr.write(platform === "darwin"
        ...
        process.stderr.write(`[moho-mcp] Keep-alive spawn failed: ${err instanceof Error ? err.message : String(err)}\n`);
        keepAliveProcess = null;
        removeScriptFile();
    }
    finally {
