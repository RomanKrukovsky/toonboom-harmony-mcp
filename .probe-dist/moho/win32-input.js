 ...
 */
function boundsGuard(xVar, yVar, label) {
    return `
...
async function runPowerShell(script) {
    const { stdout, stderr } = await execFileAsync("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { timeout: 15_000 });
    const trimmed = stdout.trim();
    if (!trimmed) {
        throw new Error(`Input command failed: ${stderr?.trim() || "no output from PowerShell"}`);
    }
    return trimmed;
}
/**
 ...
 */
const VK_MAP = Object.assign(Object.create(null), {
    // Modifiers
    ctrl: 0x11,
    control: 0x11,
    shift: 0x10,
 ...
 */
function lookupVk(part) {
    if (!Object.prototype.hasOwnProperty.call(VK_MAP, part)) {
        throw new Error(`Unknown key: "${part}". Supported keys: ${Object.keys(VK_MAP)
...
export async function sendKeys(keys) {
    ...
    for (const part of parts) {
        ...
        if (mainKey !== undefined) {
            throw new Error(`Invalid shortcut "${keys}": found more than one non-modifier key. Send one key at a time.`);
        }
