...
let _captureAppWindow;
let _loaded = false;
function checkScreenshotsEnabled() {
    if (config.moho?.enableScreenshots !== true) {
        throw new Error("SECURITY WARNING: Window screenshot capture is disabled by default. " +
...
async function loadBackend() {
    ...
    if (platform === "win32") {
        ...
        throw new Error(`Window capture is not supported on ${platform}. ` +
