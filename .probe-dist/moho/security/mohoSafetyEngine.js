 ...
 */
const PATH_PARAM_KEYS = ["outputPath", "path"];
/**
...
export class MohoSafetyEngine {
    ...
    maxExecutedKeys = 4096;
    /** Throw if method is not in the allow-list. */
    validateMethodWhitelist(method) {
        if (typeof method !== "string" || !ALLOWED_METHODS.has(method)) {
            throw new MohoSecurityError(`Method '${method}' is not in the allowed Moho safety whitelist. Arbitrary code execution is prohibited.`, "method_not_whitelisted");
        }
    }
     ...
     */
    createExecutionPlan(correlationId, projectRevision, steps) {
        if (!Array.isArray(steps) || steps.length === 0) {
            throw new MohoValidationError("An execution plan must contain at least one step.", "plan_empty");
        }
        const planSteps = steps.map((s, idx) => {
        ...
        const plan = {
            ...
            summary: `Planned ${planSteps.length} operations. ${requiresConfirmation ? "Requires explicit confirmation via previewHash." : "Safe to auto-execute."}`,
        };
     ...
     */
    validatePreviewConfirmation(method, params, previewHash) {
        if (!this.isDestructive(method))
            return;
        if (!previewHash) {
            throw new MohoValidationError(`Destructive operation '${method}' requires a valid 'previewHash' generated from a prior plan preview. Passing 'confirm: true' alone is prohibited.`, "preview_hash_required");
        }
        const cachedPlan = this.getPlan(previewHash);
        if (!cachedPlan) {
            throw new MohoValidationError(`Invalid, unknown, or expired previewHash '${previewHash}'. Generate a fresh execution plan preview first.`, "preview_hash_invalid");
        }
        const methodSteps = cachedPlan.steps.filter((s) => s.method === method);
        if (methodSteps.length === 0) {
            throw new MohoValidationError(`previewHash '${previewHash}' was not generated for method '${method}'.`, "preview_hash_method_mismatch");
        }
        ...
        const matched = methodSteps.some((step) => {
            ...
            for (const [key, value] of Object.entries(supplied)) {
                ...
                if (canonicalize(approved[key]) !== canonicalize(value)) {
                    conflicts.push(`'${key}' approved as ${canonicalize(approved[key])} but requested as ${canonicalize(value)}`);
                    return false;
                }
            }
            return true;
        ...
        if (!matched) {
            throw new MohoValidationError(`previewHash '${previewHash}' does not authorize this exact '${method}' call: ` +
                `${conflicts.join("; ")}. Generate a fresh plan for the operation you intend to run.`, "preview_hash_params_mismatch");
        }
    }
    /**
     ...
     */
    validateBatchSafety(operations, allowedDirs) {
        if (!Array.isArray(operations) || operations.length === 0) {
            throw new MohoValidationError("Batch must be a non-empty array of operations.", "batch_empty");
        }
        if (operations.length > config.moho.maxBatchSize) {
            throw new MohoValidationError(`Batch operation limit exceeded: ${operations.length} ops requested (max allowed: ${config.moho.maxBatchSize}).`, "batch_too_large");
        }
        for (const op of operations) {
            if (typeof op !== "object" || op === null) {
                throw new MohoValidationError("Each batch operation must be a table { method, params }.", "batch_op_malformed");
            }
            this.validateMethodWhitelist(op.method);
            if (this.isDestructive(op.method)) {
                throw new MohoValidationError(`Destructive operation '${op.method}' cannot run inside a batch. A batch cannot carry ` +
                    `per-operation previewHash confirmation, so this would bypass the confirmation gate. ` +
                    `Run it through its dedicated tool with its own previewHash.`, "batch_destructive_forbidden");
            }
            // Nested batches would recurse past these checks with a fresh op list.
            if (op.method === "batch.execute") {
                throw new MohoValidationError("Nested 'batch.execute' is not allowed; flatten the operations into a single batch.", "batch_nested_forbidden");
            }
    ...
    validatePathSandbox(targetPath, allowedDirs) {
        if (!Array.isArray(allowedDirs) || allowedDirs.length === 0) {
            throw new MohoSecurityError(`No allowed directories configured; cannot resolve '${targetPath}'.`, "sandbox_unconfigured");
        }
        ...
        if (!allowed) {
            throw new MohoSecurityError(`Access denied: Path '${targetPath}' is outside authorized sandbox directories (${allowedDirs.join(", ")}).`, "sandbox_violation");
        }
        return resolved;
    }
    /** Check an interpolation mode against the supported set. */
    validateInterpolationMode(mode) {
        if (typeof mode !== "string" || !INTERPOLATION_MODES.includes(mode)) {
            throw new MohoValidationError(`Unsupported interpolation mode: ${String(mode)}. Valid: ${INTERPOLATION_MODES.join(", ")}.`, "invalid_interp_mode");
        }
    }
    /**
