...
import { MohoIpcError } from "./moho-client.js";
...
function checkScreenshotPermission() {
    if (!config.moho.enableScreenshots) {
        throw new Error("SECURITY: Read-only screenshot capture is disabled by default. " +
            "Set MOHO_MCP_ENABLE_SCREENSHOTS=true in your environment to enable document_screenshot.");
    }
}
function checkUiAutomationPermission() {
    if (!config.uiAutomation.enabled) {
        throw new Error("SECURITY: Level 2 UI Automation is disabled by default. " +
...
export function registerTools(server, client) {
    const safeSend = async (method, params, options = {}) => {
        ...
        try {
            ...
            if (write) {
                ...
                if (safetyEngine.isDestructive(method))
                    SLO.destructiveConfirmations.inc();
            }
            else {
            ...
            SLO.ipcRoundtrip.inc(1, duration);
            if (write)
                SLO.writeErrors.inc(1, duration);
            else
                SLO.readErrors.inc(1, duration);
            if (err instanceof MohoIpcError) {
                const code = err.code;
                if (code === -32011 || code === -32012 || code === -32015)
                    SLO.readErrors.recordError();
                if (code === -32014)
                    SLO.readErrors.recordError();
            }
            throw err;
    ...
    /* 1. Document tools                                                     */
    /* ====================================================================== */
    registerTool(server, "document_getInfo", "Get information about the currently open MOHO document (name, path, dimensions, frame range, FPS)", {}, async () => {
        try {
    ...
    registerTool(server, "document_getLayers", "Get a list of all top-level layers (with nested children) in the current MOHO document", {}, async () => {
        try {
    ...
    registerTool(server, "document_setFrame", "Set current timeline frame in MOHO document", { frame: z.number().int().min(0).describe("Frame number to switch to") }, async ({ frame }) => {
        try {
    ...
    registerTool(server, "document_screenshot", "Render the current MOHO document frame to a PNG. Requires MOHO_MCP_ENABLE_SCREENSHOTS=true.", {
    ...
    registerTool(server, "document_createLayer", "Create a new top-level layer in the document", {
    ...
    registerTool(server, "document_save", "Save the current document. DESTRUCTIVE — requires previewHash from workflow.createExecutionPlan preview.", {
        ...
        try {
            return successContent(await safeSend("document.save", { path: args.path, format: args.format }, { previewHash: args.previewHash }));
        }
        catch (err) {
            return errorContent(err);
        }
    });
    registerTool(server, "document_diagnose", "Run a full document health scan (missing media, broken refs, render budget).", {}, async () => {
        try {
    ...
    registerTool(server, "document_render", "Render a frame range to a PNG/JPG/EXR sequence", {
        ...
        try {
            ...
            return successContent(await safeSend("document.render", {
                ...args,
    ...
    /* 2. Layer tools                                                        */
    /* ====================================================================== */
    registerTool(server, "layer_getProperties", "Get detailed properties of a specific layer (type, visibility, transform, opacity, blend, children/point/bone counts)", { layerId: z.number().int().min(0).describe("The absolute layer ID") }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "layer_getChildren", "Get direct child layers of a group layer", { layerId: z.number().int().min(0).describe("The parent group layer ID") }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "layer_getBones", "Get all bones in a bone layer", { layerId: z.number().int().min(0).describe("The bone layer ID") }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "layer_setTransform", "Set translation, rotation, or scale for a layer at a frame", {
    ...
    registerTool(server, "layer_setVisibility", "Set layer visibility at an optional frame", {
        layerId: z.number().int().min(0),
        visible: z.boolean(),
        frame: z.number().int().min(0).optional(),
    }, async (args) => {
    ...
    registerTool(server, "layer_setOpacity", "Set layer opacity (0.0 to 1.0) at an optional frame", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "layer_setName", "Rename a layer", { layerId: z.number().int().min(0), name: z.string().min(1).max(256) }, async (args) => {
        try {
    ...
    registerTool(server, "layer_selectLayer", "Set the active selection to a layer", { layerId: z.number().int().min(0) }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "layer_reorder", "Move a layer to a new index in its parent group", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "layer_setBlendMode", "Set layer blend mode at an optional frame", {
        layerId: z.number().int().min(0),
        blendMode: z.enum([
            "normal",
            "multiply",
    ...
    registerTool(server, "layer_setMask", "Toggle masking on a layer (optionally referencing another layer as mask source)", {
        layerId: z.number().int().min(0),
        masked: z.boolean(),
    ...
    registerTool(server, "layer_createGroup", "Create a new group layer and optionally reparent existing layers into it", {
    ...
    registerTool(server, "layer_createSwitch", "Create a switch layer (cycles through options like a state machine)", {
        name: z.string().min(1).max(256),
    ...
    registerTool(server, "layer_delete", "Delete a layer. DESTRUCTIVE — requires previewHash.", {
        layerId: z.number().int().min(0),
        previewHash: z.string().min(1),
    }, async (args) => {
        try {
            return successContent(await safeSend("layer.delete", { layerId: args.layerId }, { previewHash: args.previewHash }));
        }
        catch (err) {
            return errorContent(err);
        }
    ...
    /* 3. Bone tools                                                         */
    /* ====================================================================== */
    registerTool(server, "bone_getProperties", "Get detailed properties of a single bone", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "bone_setTransform", "Set position, angle, or scale for a bone at a specific frame", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    ...
    registerTool(server, "bone_selectBone", "Select a specific bone in a bone layer", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    }, async (args) => {
        try {
    ...
    registerTool(server, "bone_createBone", "Create a new bone in a bone layer with an optional parent", {
        layerId: z.number().int().min(0),
        name: z.string().min(1).max(256),
        position: z.object({ x: z.number(), y: z.number() }).optional(),
        angle: z.number().optional(),
    ...
    registerTool(server, "bone_setConstraints", "Set min/max angle, control flags on a bone", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    ...
    registerTool(server, "bone_setTarget", "Bind a bone to a target layer (or another bone) for IK-driven animation", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    ...
    registerTool(server, "bone_setParent", "Re-parent a bone within the same bone layer", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    ...
    /* 4. Animation tools                                                    */
    /* ====================================================================== */
    registerTool(server, "animation_getKeyframes", "Get keyframe data for a specific animation channel on a layer", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "animation_getFrameState", "Get the full animation state of a layer at a specific frame", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "animation_setKeyframe", "Create or update a keyframe on an animation channel", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "animation_setMultiKeyframe", "Set multiple keyframes on a channel in a single IPC round-trip (much faster than N setKeyframe calls)", {
        layerId: z.number().int().min(0),
        channel: z.string().min(1),
        keyframes: z
            .array(z.object({
    ...
    registerTool(server, "animation_deleteKeyframe", "Delete a keyframe from a channel at a frame. DESTRUCTIVE — requires previewHash.", {
        layerId: z.number().int().min(0),
        channel: z.string().min(1),
        frame: z.number().int().min(0),
        previewHash: z.string().min(1),
        ...
        try {
            return successContent(await safeSend("animation.deleteKeyframe", { layerId: args.layerId, channel: args.channel, frame: args.frame }, { previewHash: args.previewHash }));
        }
        catch (err) {
            return errorContent(err);
        }
    });
    registerTool(server, "animation_setInterpolation", "Set interpolation mode on a keyframe (linear, smooth, ease_in/out, step, bezier, noisy, cycle)", {
        layerId: z.number().int().min(0),
        channel: z.string().min(1),
        frame: z.number().int().min(0),
    ...
    registerTool(server, "animation_getPointAnim", "Get point-level animation data (e.g. shape morph keyframes) on a vector layer", {
        layerId: z.number().int().min(0),
    ...
    /* 5. Mesh tools                                                         */
    /* ====================================================================== */
    registerTool(server, "mesh_getPoints", "Get all mesh points (vertices) on a vector layer", { layerId: z.number().int().min(0) }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "mesh_getShapes", "Get all shapes (filled regions) on a vector layer", { layerId: z.number().int().min(0) }, async ({ layerId }) => {
        try {
    ...
    registerTool(server, "mesh_createPoint", "Add a single vector point to a mesh with optional bezier handles", {
        layerId: z.number().int().min(0),
        x: z.number(),
        y: z.number(),
    ...
    registerTool(server, "mesh_createBezier", "Create a multi-point bezier curve as a new shape on a vector layer", {
        layerId: z.number().int().min(0),
        points: z
            .array(z.object({
            x: z.number(),
    ...
    registerTool(server, "mesh_weld", "Weld two adjacent points into one. DESTRUCTIVE — requires previewHash.", {
        layerId: z.number().int().min(0),
        ...
        try {
            return successContent(await safeSend("mesh.weld", { layerId: args.layerId, pointIndexA: args.pointIndexA, pointIndexB: args.pointIndexB }, { previewHash: args.previewHash }));
        }
        catch (err) {
            return errorContent(err);
        }
    });
    registerTool(server, "mesh_setFill", "Toggle fill on a shape and set its color (RGBA, 0..1)", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "mesh_setStroke", "Toggle stroke on a shape and set its width and color (RGBA, 0..1)", {
        layerId: z.number().int().min(0),
        shapeIndex: z.number().int().min(0),
        hasStroke: z.boolean(),
    ...
    registerTool(server, "mesh_setGradient", "Configure a linear gradient fill on a shape", {
        layerId: z.number().int().min(0),
        shapeIndex: z.number().int().min(0),
        enabled: z.boolean(),
        startColor: z
    ...
    registerTool(server, "mesh_setCurvature", "Set bezier handle offsets for an existing point (controls curvature at that point)", {
        layerId: z.number().int().min(0),
        pointIndex: z.number().int().min(0),
        bezierInX: z.number(),
        bezierInY: z.number(),
    ...
    /* 6. Batch execution                                                    */
    /* ====================================================================== */
    registerTool(server, "batch_execute", "Execute multiple operations sequentially within a single Lua round-trip. Strongly preferred for ≥2 ops.", {
        operations: z
            .array(z.object({
        ...
        try {
            safetyEngine.validateBatchSafety(args.operations, [process.cwd(), os.tmpdir()]);
    ...
    /* ====================================================================== */
    registerTool(server, "workflow_applyLipSync", "Apply a list of phoneme keyframes (one per frame) to a vector layer's mouth shape", {
        layerId: z.number().int().min(0),
        phonemes: z
            .array(z.object({
            frame: z.number().int().min(0),
    ...
    registerTool(server, "workflow_createSmartBone", "Define a Smart Bone action: a range of frames and parameters that drive a bone from the user's input", {
        layerId: z.number().int().min(0),
        boneId: z.number().int().min(0),
    ...
    registerTool(server, "workflow_duplicateLayerTree", "Deep-clone a layer (with all children and keyframes) under a new name", {
        layerId: z.number().int().min(0),
    ...
    registerTool(server, "workflow_batchRender", "Render a list of scene configurations in sequence. DESTRUCTIVE — requires previewHash.", {
        scenes: z
            .array(z.object({
        ...
        try {
            ...
            return successContent(await safeSend("workflow.batchRender", { scenes: args.scenes }, { previewHash: args.previewHash }));
        }
        catch (err) {
            return errorContent(err);
        }
    });
    registerTool(server, "workflow_projectDiagnostics", "Project-wide health scan: missing media, broken refs, render budget, keyframe density, palette issues", {}, async () => {
        try {
    ...
    registerTool(server, "workflow_createCharacterRig", "Build a starter character rig scaffold (bone layer + named bones) ready for the user to bind to art layers", {
    ...
    /* ====================================================================== */
    registerTool(server, "document_screenshot_window", "Capture the live MOHO application window (OS-level) to a PNG. Requires MOHO_MCP_ENABLE_SCREENSHOTS=true.", {
        outputPath: z.string().optional(),
    }, async ({ outputPath }) => {
        try {
            checkScreenshotPermission();
    ...
    registerTool(server, "input_mouseClick", "Send a mouse click within the Moho window bounds. Requires MOHO_MCP_ENABLE_UI_AUTOMATION=true.", {
        x: z.number(),
        y: z.number(),
    ...
    registerTool(server, "input_mouseDrag", "Send a mouse drag from one point to another within the Moho window. Requires MOHO_MCP_ENABLE_UI_AUTOMATION=true.", {
        startX: z.number(),
        startY: z.number(),
        endX: z.number(),
        endY: z.number(),
    ...
    registerTool(server, "input_sendKeys", "Send a keyboard shortcut to the Moho window. Requires MOHO_MCP_ENABLE_UI_AUTOMATION=true.", {
    ...
    /* ====================================================================== */
    registerTool(server, "system_getCapabilities", "Return Moho and bridge capability manifest", {}, async () => {
        try {
            return successContent({
                mohoVersion: "14.0",
    ...
    registerTool(server, "system_diagnose", "Diagnostic check of IPC spooling, file system permissions, and bridge health", {}, async () => {
        try {
            await ensureConnected(client);
    ...
    registerTool(server, "system_sloSnapshot", "Snapshot in-process SLO counters (read/write/timeout/render counts and percentiles)", {}, async () => {
        try {
            return successContent({
    ...
    if (config.moho.enableLegacyAliases) {
        ...
        for (const { alias, primary } of aliasMap) {
            registerTool(server, alias, `[DEPRECATED LEGACY ALIAS] Use '${primary}' instead. Enabled via MOHO_MCP_ENABLE_LEGACY_ALIASES=true.`, { params: z.record(z.unknown()).optional() }, async (args) => {
                try {
                    const payload = args?.params ?? {};
                    const result = await safeSend(primary.replace("_", "."), payload);
