 ...
 */
import { promises as fs, openSync, closeSync, writeSync, fstatSync } from "node:fs";
...
function emitInternalDiagnostic(message) {
    try {
...
function writeAllSync(fd, text) {
    ...
    while (offset < buf.length) {
        ...
        if (written <= 0)
            break; // cannot make progress; drop the remainder
        offset += written;
    }
}
function emit(fields) {
    if (LEVEL_ORDER[fields.level] < LEVEL_ORDER[minLevel])
        return;
    let line;
    try {
...
function isStdoutAliasPath(filePath) {
    const normalized = filePath.trim().replace(/\\/g, "/").toLowerCase();
    if (STDOUT_ALIASES.has(normalized))
        return true;
    if (STDOUT_ALIASES.has(normalized.replace(/\//g, "\\")))
        return true;
    return false;
}
/**
...
function pointsAtStdout(fd) {
    ...
    if (ours.dev === 0 && ours.ino === 0)
        return false; // unidentifiable; avoid false positive
 ...
 */
export async function attachFileSink(filePath) {
    const run = sinkMutex.then(async () => {
        ...
        if (isStdoutAliasPath(filePath) || isStdoutAliasPath(resolved)) {
            emitInternalDiagnostic(`attachFileSink refused: ${filePath} resolves to stdout, which is reserved for the MCP protocol stream. Logs stay on stderr.`);
            return false;
        }
        let fd = null;
        try {
        ...
        if (pointsAtStdout(fd)) {
            ...
            emitInternalDiagnostic(`attachFileSink refused: ${resolved} is the same file as stdout, which is reserved for the MCP protocol stream. Logs stay on stderr.`);
            fileSinkFd = null;
            fileSinkPath = null;
            return false;
        }
...
function detachFileSinkUnlocked() {
    if (fileSinkFd !== null) {
        try {
