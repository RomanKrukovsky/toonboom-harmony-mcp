 ...
 */
function assertUiAutomationEnabled() {
    const enabled = config.uiAutomation?.enabled === true && config.moho?.enableUiAutomation === true;
    if (!enabled) {
        throw new Error("SECURITY: Level 2 UI Automation is disabled. " +
 ...
 */
async function runJXAWithArgs(script, args) {
    const { stdout, stderr } = await execFileAsync("osascript", ["-l", "JavaScript", "-e", script, "--", ...args], { timeout: 15_000 });
 ...
 */
async function getMohoWindowState() {
    let raw;
    try {
        ...
        throw new Error(`Unable to read the Moho window. Is Moho running and is Accessibility ` +
    ...
    if (state.width <= 0 || state.height <= 0) {
        throw new Error(`Moho window has no usable area (${state.width}x${state.height}); it may be minimized.`);
    }
    return {
...
function assertForeground(state) {
    if (config.uiAutomation?.enforceForeground !== true)
        return;
    if (!/moho/i.test(state.frontmost)) {
        throw new Error(`Refusing to send input: Moho is not the frontmost application ` +
 ...
 */
function toScreenPoint(state, x, y, label) {
    if (config.uiAutomation?.boundedByMohoWindow === true) {
        if (x < 0 || y < 0 || x >= state.width || y >= state.height) {
            throw new Error(`Refusing to send input: ${label} (${x}, ${y}) is outside the Moho ` +
...
export async function sendMouseClick(x, y, button = "left", clickType = "single") {
    ...
    try {
        ...
        throw new Error(`Mouse click failed. The 'cliclick' utility is required for macOS mouse ` +
...
export async function sendMouseDrag(startX, startY, endX, endY, button = "left", steps = 10) {
    ...
    try {
        await execFileAsync("cliclick", [`dd:${start.screenX},${start.screenY}`, `du:${end.screenX},${end.screenY}`], { timeout: 10_000 });
    }
    catch {
 ...
 */
const MAC_KEY_MAP = Object.assign(Object.create(null), {
    // Letters a-z
...
export async function sendKeys(keys) {
    ...
    if (parts.length > MAX_SHORTCUT_PARTS) {
        throw new Error(`Invalid shortcut "${keys}": at most ${MAX_SHORTCUT_PARTS} "+"-separated parts are allowed.`);
    }
    const modifiers = [];
    let mainKeyName;
    for (const part of parts) {
        ...
        if (!Object.prototype.hasOwnProperty.call(MAC_KEY_MAP, part)) {
            throw new Error(`Unknown key: "${part}". Supported keys: ${Object.keys(MAC_KEY_MAP)
                .filter((k) => !MODIFIER_KEYS.has(k))
                .join(", ")}`);
        }
        if (mainKeyName !== undefined) {
            throw new Error(`Invalid shortcut "${keys}": found more than one non-modifier key ` +
