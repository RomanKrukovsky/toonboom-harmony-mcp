...
export const mohoResources = [
    ...
    },
    {
        ...
        description: "Explains how to obtain live Moho document state. Resources cannot read it; the document tools can.",
        mimeType: "application/json",
        read: async () => JSON.stringify({
            liveStateAvailableHere: false,
            reason: "MCP resources in this server are static/config-only: they are not given a Moho IPC client, " +
    ...
    },
    {
        ...
        read: async () => {
            ...
            const clientLockPresent = ipcDirPresent && existsSync(path.join(config.moho.ipcDir, "client_lock.json"));
            return JSON.stringify({
                ipcDirPresent,
                clientLockPresent,
                ipcStatusIsProbed: false,
                ipcStatusNote: "Liveness is not probed here. Use moho.system.diagnose for an end-to-end IPC check.",
    ...
    },
    {
        ...
        read: async () => JSON.stringify({
            ...
            level2Control: "UI Automation (disabled by default, requires MOHO_MCP_ENABLE_UI_AUTOMATION=true)",
            ...
            destructiveMethods: "Require valid previewHash from safety engine plan preview (60s TTL)",
            ipcLocking: "Single-consumer client_lock.json with heartbeat refresh; stale lock auto-stolen after TTL",
