...
export class HostConfigError extends Error {
    raw;
    constructor(raw) {
        super(`Неизвестное значение ${HOST_ENV_VAR}=${JSON.stringify(raw)}. ` +
...
export function activeHost() {
    if (cached === undefined)
        cached = parseHost(process.env[HOST_ENV_VAR]);
    return cached;
}
