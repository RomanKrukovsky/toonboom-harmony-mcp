...
export const EVIDENCE_REQUIRED_FROM = 'offline_verified';
/** At or above this level, the models used must be named with hashes, plus measurements. */
export const MODEL_EVIDENCE_REQUIRED_FROM = 'real_model_verified';
export function levelRank(level) {
    return VERIFICATION_LEVELS.indexOf(level);
}
export function atOrAbove(level, floor) {
...
export function validateRegistry(registry, repoRoot = process.cwd()) {
    ...
    for (const capability of registry.capabilities) {
        ...
        const add = (rule, detail) => violations.push({ capabilityId: id, rule, detail });
        if (seen.has(id))
            add('unique-id', `duplicate capabilityId "${id}"`);
        seen.add(id);
        ...
        if ((capability.verificationLevel === 'not_implemented' || capability.verificationLevel === 'unaudited') &&
...
export function sha256OfFile(target) {
 ...
 */
export function isFabricatedMedia(target) {
...
export function validateEvidenceBundle(bundleDir) {
    const violations = [];
    const add = (rule, detail) => violations.push({ bundle: bundleDir, rule, detail });
    ...
    if (fs.existsSync(hashesPath)) {
        ...
        for (const [relative, expected] of Object.entries(hashes)) {
            ...
            if (actual !== expected) {
                add('hash-matches', `${relative}: recorded ${expected.slice(0, 16)}… but file hashes to ${actual.slice(0, 16)}…`);
            }
        }
    }
    return violations;
...
export function listCommittedEvidenceBundles(repoRoot = process.cwd()) {
 ...
 */
export function scanForFabricatedMedia(root, options = {}) {
    ...
    while (stack.length > 0 && inspected < limit) {
        ...
        for (const entry of entries) {
            ...
            try {
                const check = isFabricatedMedia(full);
                if (check.fabricated)
                    found.push({ file: full, reason: check.reason });
            }
            catch {
