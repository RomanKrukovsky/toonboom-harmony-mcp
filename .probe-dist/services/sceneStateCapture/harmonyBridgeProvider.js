...
export class HarmonyBridgeSceneStateProvider {
    timeoutMs;
    probeScenePath;
    source = 'harmony_python_bridge';
    /**
     ...
     */
    constructor(timeoutMs = 60000, probeScenePath) {
        this.timeoutMs = timeoutMs;
        this.probeScenePath = probeScenePath;
    }
    async describe() {
        try {
            const detect = await HarmonyPython.runCommand('detect', this.probeScenePath ? { projectPath: this.probeScenePath } : {}, this.timeoutMs);
            const probeError = detect?.capabilities?.session_probe_error;
    ...
    async captureFull(ctx) {
        ...
        const nodePaths = Array.isArray(nodeList?.nodes) ? nodeList.nodes.map((n) => String(n)) : [];
        const nodes = [];
        const nodeAttributes = [];
        const layerTypeByPath = new Map();
    ...
    async call(command, args) {
        ...
        if (response?.status === 'error' || response?.error === true) {
            throw new HarmonyError('CAPTURE_STATE_PROVIDER_UNAVAILABLE', `Harmony bridge command "${command}" failed: ${response?.message ?? 'unknown error'}`, { command, details: response?.details });
        }
        return response;
    }
}
/** True when an attribute keyword denotes a peg/transformation channel. */
export function isTransformAttribute(attribute) {
