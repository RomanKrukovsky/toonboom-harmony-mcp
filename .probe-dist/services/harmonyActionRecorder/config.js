...
export function loadRecorderConfig(overrides = {}) {
    const categories = {
    ...
    const base = {
        artifactRoot: path.resolve(process.env.HARMONY_CAPTURE_ARTIFACT_ROOT ||
 ...
 */
export function resolveAllowedScenePath(scenePath, cfg) {
    const resolved = path.resolve(scenePath);
    const roots = (cfg.allowedSceneRoots.length > 0 ? cfg.allowedSceneRoots : harmonyConfig.allowedRoots).map(root => path.resolve(root));
    const permitted = roots.some(root => {
        const relative = path.relative(root, resolved);
        return relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative));
    });
    if (!permitted) {
        throw new HarmonyError('CAPTURE_SCENE_ROOT_NOT_ALLOWED', `Scene path is outside every allowed capture root.`, { allowedRoots: roots });
    }
    return resolved;
}
