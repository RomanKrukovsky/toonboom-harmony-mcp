...
import { canonicalHash } from '../../schemas/harmonyActionDataset.js';
...
const MUTABLE_ARTIFACTS = new Set([SESSION_ARTIFACTS.session, SESSION_ARTIFACTS.hashes]);
export class CaptureSessionStore {
    artifactRoot;
    sessionId;
    sessionDir;
    constructor(artifactRoot, sessionId) {
    ...
    create() {
        if (fs.existsSync(this.sessionDir)) {
            throw new HarmonyError('CAPTURE_ARTIFACT_IMMUTABLE', `Evidence directory for session "${this.sessionId}" already exists; sessions are never overwritten.`, { sessionDir: this.sessionDir });
        }
    ...
    writeJson(name, value) {
        ...
        if (fs.existsSync(target) && !MUTABLE_ARTIFACTS.has(name)) {
            throw new HarmonyError('CAPTURE_ARTIFACT_IMMUTABLE', `Artifact "${name}" of session "${this.sessionId}" already exists and cannot be rewritten.`, { path: target });
        }
    ...
    readJson(name) {
        ...
        try {
            ...
            throw new HarmonyError('CAPTURE_SESSION_INVALID_STATE', `Artifact "${name}" of session "${this.sessionId}" is not readable JSON: ${error.message}`, { path: target });
        }
    }
    ...
    readEvents() {
        ...
        if (!fs.existsSync(target))
            return { events: [], truncatedTailBytes: 0 };
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            try {
                ...
                if (i === lines.length - 1) {
                    ...
                    throw new HarmonyError('CAPTURE_SESSION_INVALID_STATE', `Event spool of session "${this.sessionId}" is corrupt at line ${i + 1}.`, { path: target });
                }
            }
        }
    ...
    writeHashes() {
        ...
        for (const name of Object.values(SESSION_ARTIFACTS)) {
            if (name === SESSION_ARTIFACTS.hashes)
                continue;
            const target = this.artifactPath(name);
            if (!fs.existsSync(target))
                continue;
        ...
        fs.writeFileSync(this.artifactPath(SESSION_ARTIFACTS.hashes), JSON.stringify({ sessionId: this.sessionId, generatedAt: new Date().toISOString(), hashes }, null, 2), 'utf-8');
        return hashes;
    }
    static listSessionIds(artifactRoot) {
