...
export const characterBibleSchema = z.object({
    ...
    turnaroundViews: z.array(z.enum(['front', 'front_3q_left', 'side_left', 'back_3q_left', 'back', 'back_3q_right', 'side_right', 'front_3q_right'])).min(1),
...
export const cameraRulesSchema = z.object({
    schemaVersion: z.literal('1.0'),
    allowedShotSizes: z.array(z.enum(['extreme_close_up', 'close_up', 'medium_close_up', 'medium_shot', 'medium_full_shot', 'full_shot', 'long_shot', 'extreme_long_shot'])).min(1),
    allowedCameraMoves: z.array(z.enum(['static', 'pan_left', 'pan_right', 'tilt_up', 'tilt_down', 'dolly_in', 'dolly_out', 'truck_left', 'truck_right', 'pedestal_up', 'pedestal_down', 'zoom_in', 'zoom_out', 'arc_left', 'arc_right', 'crane_up', 'crane_down'])).min(1),
...
export const showBibleSchema = z.object({
    ...
    allowedDeformations: z.array(z.enum(['peg_transform', 'curve_deformer', 'envelope_deformer', 'bone_deformer', 'drawing_substitution', 'frame_by_frame_vector'])).min(1),
...
// Validation helpers
// ─────────────────────────────────────────────────────────────────────────────
export function assertShowBibleVersion(doc) {
    if (!doc || typeof doc.schemaVersion !== 'string') {
        throw new Error('show_bible.json missing required "schemaVersion" field');
    }
    const [maj, min] = doc.schemaVersion.split('.').map((n) => parseInt(n, 10));
