...
export const performancePlanSchema = z.object({
    ...
    if (plan.eventCount !== plan.events.length)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'eventCount does not match events.length' });
    for (const event of plan.events)
        if (event.endTime <= event.startTime)
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: `${event.eventId} has invalid duration` });
});
