...
async function executeCcScript(script, dryRun, operationName) {
...
export const controlCenterTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            if (!config.allowRawScripts) {
                throw new HarmonyError('PATH_NOT_ALLOWED', 'Прямой запуск Qt-скриптов отключен в настройках конфигурации (HARMONY_ALLOW_RAW_SCRIPTS=false).');
            }
    ...
    defineTool({
        ...
        handler: async (args) => {
            const checkedPackagePath = verifyPathAccess(args.packagePath);
            const script = QtScriptBuilder.buildExportScenePackage(args.environmentName, args.jobName, args.sceneName, args.versionNumber, checkedPackagePath);
