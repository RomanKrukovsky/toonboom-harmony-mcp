...
function registryPath(registryDir) {
    return path.join(verifyPathAccess(registryDir ?? path.join(process.cwd(), DEFAULT_REGISTRY_DIR)), REGISTRY_FILENAME);
}
...
function saveRegistry(registry, file) {
...
function transparentPngBytes() {
    return Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYGBgAAAABQABijPjAAAAAElFTkSuQmCC', 'base64');
}
/** Locate the asset_license.json governing a file, nearest-first. */
function findLicenseFor(filePath) {
...
export const assetRegistryTools = [
    defineTool({
        ...
        handler: async (args) => {
            ...
            const byType = requiredAssets.reduce((acc, asset) => {
    ...
    defineTool({
        ...
        handler: async (args) => {
            const { generateCharacterTurnaround, generateBackground } = await import('../adapters/backends/imageBackend.js');
            const result = args.assetType === 'background'
                ? await generateBackground(args.assetId, args.prompt, args.outputPath)
                : await generateCharacterTurnaround(args.assetId, args.view ?? 'front', args.prompt, args.outputPath);
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (args.assetType)
                records = records.filter(r => r.assetType === args.assetType);
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            let parsedLicense;
            try {
                parsedLicense = assetLicenseSchema.safeParse(JSON.parse(fs.readFileSync(licenseFile, 'utf-8')));
            }
            catch (err) {
                return {
                    ...createStandardExecutionResult({
            ...
            if (!contractPresent)
                blockers.push(`Договор не найден: ${license.contractPath}`);
            if (digestMatches === false)
                blockers.push('Файл изменён после подписания лицензии (sha256 не совпадает).');
            return {
                ...createStandardExecutionResult({
