...
async function runDiagnosticsInternal(tempDir) {
    ...
    const warnings = [];
    if (!configured)
        blockingIssues.push('Toon Boom Harmony binary (HARMONY_BIN) is not configured.');
    else if (!exists)
        blockingIssues.push(`Toon Boom Harmony binary file does not exist at path: "${binPath}".`);
    else if (!executable)
        blockingIssues.push(`Toon Boom Harmony binary file is not executable at path: "${binPath}".`);
    if (!packagesPathExists)
        warnings.push('Toon Boom Harmony Python packages path (HARMONY_PYTHON_PACKAGES) is not configured or does not exist.');
    if (!canImportToonBoomHarmony)
        blockingIssues.push('Failed to import ToonBoom.harmony Python package.');
...
export const sceneTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const matches = nodes.filter((n) => n.toLowerCase().includes(args.query.toLowerCase()));
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (!isHarmonyReady) {
                ...
                fs.writeFileSync(path.join(reportsDir, 'real_smoke_test_report.json'), JSON.stringify(failedReport, null, 2), 'utf8');
                return failedReport;
            }
            const scenePlan = {
                sceneName: 'SC_001',
            ...
            if (!fs.existsSync(reportsDir))
                fs.mkdirSync(reportsDir, { recursive: true });
            fs.writeFileSync(path.join(reportsDir, 'real_smoke_test_report.json'), JSON.stringify(report, null, 2), 'utf8');
            return report;
        }
    }),
    defineTool({
