...
const SEVERITY_PENALTY = { critical: 25, major: 10, minor: 3 };
...
export const qualityEngineTools = [
    defineTool({
        ...
        handler: async (args) => {
            ...
            const issuePenalty = knownIssues.reduce((sum, issue) => sum + (SEVERITY_PENALTY[issue.severity ?? 'minor'] ?? 3), 0);
            const finalScore = Math.max(0, scores.total - issuePenalty);
            // Derive structural findings from the scene itself.
            const findings = [];
            if (!scene.location)
                findings.push('Не задана локация — фон не может быть собран.');
            if (!scene.characters || scene.characters.length === 0)
                findings.push('В сцене нет персонажей.');
            if (!scene.durationFrames || scene.durationFrames <= 0)
                findings.push('Длительность не задана.');
            else if (scene.durationFrames < 12)
                findings.push(`Длительность ${scene.durationFrames} кадров — меньше 0.5с при 24fps.`);
            if (!scene.cameraNotes)
                findings.push('Нет заметок по камере — кадрирование не определено.');
            if (!scene.mood || scene.mood === 'generic')
                findings.push('Настроение не задано — актёрская игра будет нейтральной.');
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            try {
                ...
                const response = await client.compareRender([
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const ordered = [...fixes].sort((a, b) => (order[a.issue.severity ?? 'minor'] ?? 2) - (order[b.issue.severity ?? 'minor'] ?? 2));
            return {
                ...createStandardExecutionResult({
                    status: 'success',
                    simulated: false,
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const queueDir = verifyPathAccess(args.reviewQueueDir ?? path.join(process.cwd(), 'output', 'review_queue'));
            ...
            const entryPath = path.join(queueDir, `${args.sceneId}_${sha256(args.sceneId + requestedAt).slice(0, 8)}.json`);
    ...
    defineTool({
        ...
        handler: async (args) => {
            const approvalsDir = verifyPathAccess(args.approvalsDir ?? path.join(process.cwd(), 'output', 'approvals'));
