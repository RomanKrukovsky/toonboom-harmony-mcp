...
export const seriesTools = [
        ...
        handler: async (args) => {
            ...
            for (let i = 1; i <= count; i++) {
                ideas.push({
                    ...
                    characters: bible.recurringCharacters.map((c) => c.name),
    ...
    defineTool({
        ...
        handler: async (args) => {
            const bible = args.seriesBible;
            const library = {
                characters: bible.recurringCharacters.map((c) => ({
                    name: c.name,
                ...
                locations: bible.recurringLocations.map((loc) => ({
                    name: loc,
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const previewTool = (await import('./onePromptTools.js')).onePromptTools.find(t => t.name === 'harmony.oneprompt.run_to_preview_episode');
