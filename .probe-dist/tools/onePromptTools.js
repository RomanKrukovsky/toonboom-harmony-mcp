...
export const onePromptTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const specs = args.characterSpecs.map((c) => synth.generateSpec(c));
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            return {
                ...
                validationErrors: errors.map((e) => e.error.format()),
                scenePlans: plans
            };
        }
    }),
    defineTool({
        ...
        handler: async (args) => {
            ...
            function writeScenePlans(plans) {
            ...
            const findTool = (name) => autopilotTools
                .find(t => t.name === name);
            ...
            while (iteration < maxIterations && currentScore < targetScore) {
                ...
                if (currentScore < targetScore) {
                    for (const s of pkg.episodePlan.scenes) {
                        if (s.durationFrames < 60)
                            s.durationFrames += 12;
                        if (!s.cameraNotes)
                            s.cameraNotes = 'medium dolly in with slight pan';
                        if (!s.mood || s.mood === 'generic')
                            s.mood = 'rising';
                    }
                    for (const a of pkg.actingPlans) {
                        if ((a.readabilityScore || 0) < 80)
                            a.readabilityScore = Math.min(100, (a.readabilityScore || 70) + 5);
                    }
                }
                ...
                if (config.onePromptIteration.stopIfNoImprovement && newScore <= currentScore)
                    break;
            ...
            if (renderPreview) {
                for (const sp of scenePlans) {
                    ...
                    try {
                        const renderRes = await renderPreview.handler({ projectPath: dummyProjectPath, outputPath: previewPath });
                        if (renderRes.outputPath)
                            previewPaths.push(renderRes.outputPath);
                    }
                    catch (e) {
            ...
            const assembledSceneCount = autopilotResults.filter((r) => r.status === 'completed').length;
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const previewTool = onePromptTools
                .find(t => t.name === 'harmony.oneprompt.run_to_preview_episode');
            const result = await previewTool.handler(args);
