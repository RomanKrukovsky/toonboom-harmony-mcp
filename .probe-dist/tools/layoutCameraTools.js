 ...
 */
function parallaxFactor(depth, cameraZ) {
...
function interpolateMove(startFrame, endFrame, from, to, easing) {
    const frames = [];
...
export const layoutCameraTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const monotonic = ordered.every((level, index) => index === 0 || Math.abs(level.apparentShift) >= Math.abs(ordered[index - 1].apparentShift) - 1e-6);
            return {
                ...createStandardExecutionResult({
                    status: 'success',
                    simulated: false,
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const durationFrames = args.durationFrames ?? 48;
            const easing = args.easing ?? 'ease_in_out';
            const keyframes = interpolateMove(1, durationFrames, { x: 0, y: 0, z: startZ }, { x: 0, y: 0, z: endZ }, easing).map(kf => ({ frame: kf.frame, z: Number(kf.z.toFixed(4)) }));
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const easing = args.easing ?? 'ease_in_out';
            const fps = args.fps ?? 24;
            const keyframes = interpolateMove(1, durationFrames, { x: args.startX, y: startY, z: 0 }, { x: args.endX, y: endY, z: 0 }, easing).map(kf => ({ frame: kf.frame, x: Number(kf.x.toFixed(4)), y: Number(kf.y.toFixed(4)) }));
            ...
            if (distance === 0)
                warnings.push('Начальная и конечная точки совпадают — движения не будет.');
            return {
                ...createStandardExecutionResult({
                    status: 'success',
                    simulated: false,
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (bytes.length > 8 && bytes[0] === 0x89 && bytes.toString('ascii', 1, 4) === 'PNG')
                ...
                format = 'jpeg';
            else if (bytes.length > 4 && bytes.toString('ascii', 0, 4) === '<svg')
                format = 'svg';
            else if (bytes.toString('ascii', 0, 5).includes('<?xml') && bytes.toString('ascii', 0, 300).includes('<svg'))
                format = 'svg';
            const warnings = [];
            if (format === 'unknown') {
    ...
    defineTool({
        ...
        handler: async (args) => {
            const libraryDir = verifyPathAccess(args.libraryDir ?? path.join(process.cwd(), 'output', 'background_library'));
