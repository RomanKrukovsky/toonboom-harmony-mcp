...
async function runNodeBridge(command, args) {
    try {
            ...
            if (!args.skipDeformerCheck) {
                try {
                    ...
                    if (auditRes.status !== 'unsupported' && auditRes.audit) {
                        ...
                        const nodeInDeformer = deformers.some((d) => {
                            const msg = typeof d === 'string' ? d : d?.message || d?.details || '';
                            return msg.includes(args.nodePath);
                        });
                        const connectedToDeformer = brokenConnections.some((c) => {
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            try {
                ...
                if (listRes.status === 'unsupported')
                    return listRes;
                const allNodes = listRes.nodes || [];
                cutterNodes = allNodes.filter((n) => {
                ...
                if (args.cutterNodePath) {
                    cutterNodes = cutterNodes.filter((n) => (n.path || n.node_path) === args.cutterNodePath);
                }
            }
            catch {
                return {
            ...
            const results = cutterNodes.map((cutter) => {
            ...
            return {
                status: 'success',
                verification: results.some((r) => r.verification === 'verified_real') ? 'verified_real' : 'implemented_unverified',
