 ...
 */
import { z } from 'zod';
import { blenderStatus, clientDir, pipelineStatus, pyStr, runPython, } from '../adapters/blenderPipeline.js';
...
function envOrError() {
    ...
    if (!b.present) {
        return fail('NO_BLENDER', b.detail, 'Install Blender or point BLENDER_BIN at the executable.');
    }
    const p = pipelineStatus();
    if (!p.present) {
        return fail('NO_PIPELINE', `pipeline files missing in ${p.dir}: ${p.missing.join(', ')}`, 'Set HARMONY_CLIENT_DIR to the harmony/client directory.');
    }
    return null;
}
export const blenderTools = [
    defineTool({
        ...
        description: 'Готовность Blender-конвейера: есть ли Blender, на месте ли питоновские модули, ' +
        ...
        handler: async () => {
            ...
            let version;
            if (b.present) {
                const r = await runPython(PRELUDE +
    ...
    defineTool({
        ...
        description: 'Проверить набор рисунков художника (PNG с альфой + parts.json) ДО рендера. ' +
        ...
        handler: async (a) => {
            ...
            if (!r.ok || !r.json) {
                return fail('CHECK_FAILED', r.stderr.slice(-800) || r.stdout.slice(-400), 'Check that parts.json exists and its images are readable.');
            }
    ...
    defineTool({
        ...
        description: 'Собрать шот: рисунки художника + тайминг + необязательный липсинк и звук -> ' +
        ...
        handler: async (a) => {
            ...
            if (a.lipsync && !a.audio && !a.allowSilentLipsync) {
                return fail('LIPSYNC_WITHOUT_AUDIO', 'lipsync was requested but no audio track was given — the mouth would ' +
                    'move against silence and the MP4 would have no sound', 'Pass audio:"/path/voice.wav", or set allowSilentLipsync:true if you ' +
...
`;
            ...
            if (r.json?.stage === 'artwork') {
                return fail('BAD_ARTWORK', `the artwork set has ${r.json.check.errors} blocking problem(s)`, 'Fix the findings below, then rebuild — rendering a broken set wastes minutes ' +
                    'for the same error.');
            }
            if (!r.ok || !r.json) {
                return fail('BUILD_FAILED', r.stderr.slice(-1000) || r.stdout.slice(-500), 'See stderr above. A Python traceback here means the spec was rejected ' +
    ...
    defineTool({
        ...
        description: 'Прогнать пробы конвейера на живом Blender: движение, тайминг из движка, звук, ' +
        ...
        handler: async (a) => {
            ...
            for (const key of want) {
                const mod = map[key];
                const r = await runPython(`import runpy, sys\nsys.argv = ["${mod}"]\n` +
