...
export const animationBlockingTools = [
    ...
    defineTool({
        ...
        description: 'Генерирует план ключевых кадров (blocking) для сцены. ' +
        ...
        handler: async (args) => {
            ...
            const interp = (style === 'snappy')
                ? { enter: 'hold', exit: 'linear' }
                : style === 'bouncy'
                    ? { enter: 'ease', exit: 'spline' }
            ...
            for (const char of characters) {
                ...
                if (charActions.length > 0) {
                    ...
                    keyframes.push({ frame: endFrame - 1, character: charName, bodyPart: 'all', pose: 'idle_end', interpolation: 'ease' });
                    thumbnailPoses.push({ frame: startFrame, description: `${charName}: начальная поза` }, { frame: endFrame - 1, description: `${charName}: финальная поза` });
                }
            }
            // Сортируем по кадру
            keyframes.sort((a, b) => a.frame - b.frame);
            const blockingPlan = { totalFrames, fps, keyframes, thumbnailPoses };
            ...
            return {
                ...
                summary: {
                    ...
                    keyframesByCharacter: characters.reduce((acc, c) => {
    ...
    defineTool({
        ...
        description: 'Применяет blocking_plan.json к открытому проекту Harmony через Python API. ' +
...
if (node.exists(nodePath)) {
  ...
  MessageLog.trace("Keyframe set: ${kf.character} @ ${kf.frame}");
    ...
    defineTool({
        ...
        description: 'Генерирует план движений камеры из описания сцены или camera_plan.json. ' +
        ...
        handler: async (args) => {
            ...
            if (style === 'static') {
                cameraKeyframes.push({ frame: 1, x: 0, y: 0, z: 0, scaleX: 1, scaleY: 1, rotation: 0, easing: 'linear', description: 'Статичная камера' });
            }
            else if (style === 'subtle') {
                cameraKeyframes.push({ frame: 1, x: 0, y: 0, z: 0, scaleX: 1, scaleY: 1, rotation: 0, easing: 'ease_in_out', description: 'Начало: камера статична' }, { frame: Math.round(totalFrames * 0.5), x: 0, y: 5, z: 0, scaleX: 1.02, scaleY: 1.02, rotation: 0, easing: 'ease_in_out', description: 'Середина: небольшой подъём' }, { frame: totalFrames, x: 0, y: 10, z: 0, scaleX: 1.05, scaleY: 1.05, rotation: 0, easing: 'ease_in_out', description: 'Финал: тихий наезд' });
            }
            else if (style === 'dynamic') {
                cameraKeyframes.push({ frame: 1, x: -20, y: 0, z: 0, scaleX: 1.1, scaleY: 1.1, rotation: 0, easing: 'ease_in_out', description: 'Начало: широкий план' }, { frame: Math.round(totalFrames * 0.3), x: 0, y: 0, z: 0, scaleX: 1, scaleY: 1, rotation: 0, easing: 'ease_in_out', description: 'Наезд на центр' }, { frame: Math.round(totalFrames * 0.7), x: 10, y: 5, z: 0, scaleX: 0.95, scaleY: 0.95, rotation: -1, easing: 'ease_out', description: 'Смещение вправо-вверх' }, { frame: totalFrames, x: 0, y: 0, z: 0, scaleX: 1.15, scaleY: 1.15, rotation: 0, easing: 'ease_in_out', description: 'Финал: крупный план' });
            }
            else { // cinematic
                cameraKeyframes.push({ frame: 1, x: 30, y: -10, z: 0, scaleX: 0.85, scaleY: 0.85, rotation: 1, easing: 'ease_in', description: 'Открывающий план: широкий угол' }, { frame: Math.round(totalFrames * 0.25), x: 10, y: 0, z: 0, scaleX: 1, scaleY: 1, rotation: 0, easing: 'ease_in_out', description: 'Камера входит' }, { frame: Math.round(totalFrames * 0.6), x: 0, y: 5, z: 0, scaleX: 1.1, scaleY: 1.1, rotation: 0, easing: 'ease_in_out', description: 'Приближение на кульминацию' }, { frame: totalFrames - Math.round(fps * 0.5), x: 0, y: 8, z: 0, scaleX: 1.2, scaleY: 1.2, rotation: 0, easing: 'ease_in', description: 'Предфинальная пауза' }, { frame: totalFrames, x: 0, y: 15, z: 0, scaleX: 1.3, scaleY: 1.3, rotation: 0, easing: 'ease_out', description: 'Финал: pull back' });
            }
...
scene.setFrameValue("camera1", ${kf.frame}, "scale.y", ${kf.scaleY});`).join('\n');
    ...
    defineTool({
        ...
        description: 'Генерирует animatic timing sheet — таблицу тайминга для всей сцены. ' +
    ...
    defineTool({
        ...
        description: 'Применяет camera plan (ключевые кадры камеры) к открытому проекту Harmony. ' +
