...
const providerModeSchema = z
    ...
    .describe('Which scene-state provider to read with. "fixture" is offline test data and is labelled as such in every artifact.');
/**
 ...
 */
export function buildProvider(mode, fixtureStatePaths, scenePath) {
    if (mode === 'fixture') {
...
export const harmonyActionRecorderTools = [
    defineTool({
        ...
        description: 'Starts a Harmony Action Recorder session: captures the "before" scene state, creates an immutable evidence directory and begins the append-only event log. Read-only with respect to the scene.',
    ...
    defineTool({
        ...
        description: 'Records the animator task description for a running session. This text is the only source of artistic intent in the exported dataset entry.',
        inputSchema: z.object({
            sessionId: z.string(),
    ...
    defineTool({
        ...
        description: 'Flushes the debounced dirty-entity queue and stores an intermediate normalized scene state for a running session.',
        inputSchema: z.object({
            sessionId: z.string(),
    ...
    defineTool({
        ...
        description: 'Reports the true status of a capture session. A session left "recording" by a crashed process is reported and persisted as "interrupted".',
        inputSchema: z.object({
    ...
    defineTool({
        ...
        description: 'Stops a session: waits for the debounce queue to settle, captures the "after" state, computes the deterministic semantic scene patch and the inverse patch, and writes the immutable artifacts.',
        inputSchema: z.object({
            sessionId: z.string(),
            renderStatus: z
    ...
    defineTool({
        ...
        description: 'Records an immutable "approved" decision for a stopped session. The scene patch is never modified by this call.',
        inputSchema: z.object({
            sessionId: z.string(),
    ...
    defineTool({
        ...
        description: 'Records an immutable "rejected" decision for a stopped session. The scene patch is never modified by this call.',
        inputSchema: z.object({
            sessionId: z.string(),
            reviewer: z.string().max(200).optional(),
            note: z.string().max(4000).optional(),
    ...
    defineTool({
        ...
        description: 'Exports a decided session as a HarmonyActionDatasetEntry: instruction, before/after state references, normalized operations, inverse operations, approval, provenance and usage restrictions.',
        inputSchema: z.object({
            sessionId: z.string(),
    ...
    defineTool({
        ...
        description: 'Compares the scene patches of two finished sessions: whether they touched the same scene, whether the patches are identical, and which operations are unique to each.',
        inputSchema: z.object({
            sessionIdA: z.string(),
            sessionIdB: z.string()
        }),
