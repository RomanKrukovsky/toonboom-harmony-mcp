...
export const renderTools = [
    defineTool({
        ...
        handler: async (args) => {
            return executeWithDryRun('queue_render', args, args.dryRun, async () => {
                ...
                const p = await tracker.get('SELECT id FROM productions LIMIT 1');
                const prodId = p ? p.id : 1;
                const queueItem = await tracker.run(`INSERT INTO tasks (shot_id, name, status, due_date) VALUES 
          (?, ?, 'Queued', CURRENT_TIMESTAMP)`, [1, `Рендеринг сцены: ${args.sceneName} V${args.versionNumber} (${args.startFrame}-${args.endFrame || 'end'})`]);
                return {
                    status: 'success',
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (!config.harmonyBin) {
                throw new HarmonyError('HARMONY_NOT_INSTALLED', 'Исполняемый файл HarmonyPremium не настроен. Пожалуйста, задайте HARMONY_BIN или HARMONY_INSTALL.');
            }
            return executeWithDryRun('render_local', args, args.dryRun, () => {
                ...
                return new Promise((resolve, reject) => {
                    execFile(config.harmonyBin, cliArgs, { timeout: config.scriptTimeoutMs * 6 }, // Даем больше времени на процесс рендеринга
                    (error, stdout, stderr) => {
                        if (error) {
                            return reject(new HarmonyError('HARMONY_NOT_INSTALLED', `Локальный рендеринг завершился ошибкой: ${error.message}. Stderr: ${stderr}`));
                        }
                        resolve({
                            status: 'success',
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const checkedDrawings = args.drawingsPaths.map((p) => verifyPathAccess(p));
            return executeWithDryRun('vectorize_queue', args, args.dryRun, async () => {
                await tracker.initialize();
                const vecItem = await tracker.run(`INSERT INTO tasks (shot_id, name, status, due_date) VALUES 
          (1, ?, 'Queued', CURRENT_TIMESTAMP)`, [`Векторизация ${checkedDrawings.length} рисунков в ${checkedProj}`]);
                return {
                    status: 'success',
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const heavyNodes = allNodes.filter((n) => {
