...
async function runDrawingBridge(command, args) {
    try {
...
export const drawingTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (res.status === 'unsupported')
                return res;
            const found = (res.drawings || []).find((d) => d.node_path === args.layerNodePath);
            return {
    ...
    defineTool({
        ...
        handler: async (args) => {
            const checkedPath = args.projectPath ? verifyPathAccess(args.projectPath) : undefined;
            let beforeDiff = [];
            let sourcePivot = null;
            try {
                const subsRes = await runDrawingBridge('list_drawings', { projectPath: checkedPath });
                if (subsRes.status !== 'unsupported') {
                    const found = (subsRes.drawings || []).find((d) => d.node_path === args.layerNodePath);
                    if (found) {
                        const subs = found.substitutions || [];
                        const sourceSub = subs.find((s) => (s.name || s.drawing_name) === args.sourceSubName);
                        if (sourceSub) {
                            sourcePivot = {
                        ...
                        for (const sub of subs) {
                            ...
                            if (subName === args.sourceSubName)
                                continue;
                            if (args.targetSubNames && !args.targetSubNames.includes(subName))
                                continue;
                            const subPivot = {
