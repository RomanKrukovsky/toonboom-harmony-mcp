...
export const autopilotTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            throw new HarmonyError('UNSUPPORTED_BY_VERSION', 'Автоматический headless-рендеринг видео в видеофайлы требует установленной лицензированной Harmony или нативной сессии рендеринга. Запись плейсхолдер-строк на диск отключена.', {
                honestLimitations: {
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            try {
                ...
                throw new HarmonyError('HARMONY_EXECUTION_FAILED', `Не удалось выполнить аудит сцены: ${err.message}`, { honestLimitations: { verification: 'requires_real_harmony' } });
            }
        }
    }),
    defineTool({
    ...
    defineTool({
        ...
        description: 'End-to-end production пайплайн: scene_plan → выполнение шагов → self_check → auto_fix → отчёт. ' +
        ...
        handler: async (args) => {
            return executeWithDryRun('autopilot.run_full_production', args, args.dryRun, async () => {
                ...
                if (args.scenePlanPath) {
                    const fullPath = path.resolve(args.scenePlanPath);
                    if (!fs.existsSync(fullPath))
                        throw new HarmonyError('SCENE_NOT_FOUND', `Файл отсутствует: ${args.scenePlanPath}`);
                ...
                report.overallStatus = report.stages.every((s) => s.status !== 'failed') ? 'success' : 'partial';
                return report;
            });
        }
    }),
    ...
    defineTool({
        ...
        description: 'Полный аудит собранной сцены: проверяет наличие всех слоёв, ноды, ключевые кадры, ' +
    ...
    defineTool({
        ...
        description: 'Автоматически исправляет проблемы из аудита (harmony.autopilot.self_check). ' +
...
async function executeStep(step, dryRun = false) {
    ...
    try {
        ...
        if (action.method === 'start_application') {
            ...
            await templateAssembly.createSceneFromTemplate(step.action.params.template, targetPath, step.action.params);
        ...
        if (!verifyRes.passed) {
            ...
            const recoverRes = await RecoveryAdapter.attemptRecovery(step.id, step.fallback.strategy, verifyRes.details, step.fallback.params);
