...
export const rigTools = [
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const deformerPlan = DeformerGenerator.generatePlan(rigPlan);
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (audit.broken_connections) {
                audit.broken_connections.forEach((c) => {
            ...
            if (audit.drawing_keyframes_pollution) {
                audit.drawing_keyframes_pollution.forEach((c) => {
            ...
            if (audit.flat_composites) {
                audit.flat_composites.forEach((c) => {
            ...
            if (audit.double_transformation) {
                audit.double_transformation.forEach((c) => {
            ...
            if (audit.negative_scale_on_deformer_pegs) {
                audit.negative_scale_on_deformer_pegs.forEach((c) => {
            ...
            if (audit.missing_kinematic_output) {
                audit.missing_kinematic_output.forEach((c) => {
            ...
            if (audit.cutter_inverted_mismatch) {
                audit.cutter_inverted_mismatch.forEach((c) => {
        ...
        handler: async (args) => {
            ...
            for (const issue of allIssues) {
                ...
                if (msg.includes('duplicate') || msg.includes('conflict') || msg.includes('multiple deformer') || msg.includes('same element')) {
                    if (args.nodePath && !msg.includes(args.nodePath))
                        continue;
                    duplicates.push({
