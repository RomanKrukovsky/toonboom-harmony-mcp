...
export const studioTools = [
    ...
    defineTool({
        ...
        description: 'ГЛАВНЫЙ ИНСТРУМЕНТ. Принимает текстовый промпт / идею сцены / раскадровку и генерирует ' +
        ...
        inputSchema: z.object({
            prompt: z.string().describe('Текстовый промпт сцены. Может быть идеей, описанием, раскадровкой или сценарным текстом. ' +
            ...
            saveToDir: z.string().optional().describe('Если указан — сохраняет scene_plan.json и все вспомогательные файлы в эту папку')
        }),
        handler: async (args) => {
            ...
            let planValidation = { status: 'skipped' };
            try {
    ...
    defineTool({
        ...
        description: 'End-to-end выполнение полного production-пайплайна по scene_plan.json. ' +
        ...
        handler: async (args) => {
            ...
            const addStage = (name, status, detail) => {
            ...
            const auditIssues = [
                ...
                ...(planObj.characters?.length === 0 ? [{
                        id: 'audit_001', severity: 'warning', category: 'structure',
                        message: 'Нет персонажей в сцене', autoFixable: false
                    }] : []),
                ...(planObj.background ? [] : [{
                        id: 'audit_002', severity: 'error', category: 'missing_asset',
            ...
            // Stage 5: Auto-fix
            const fixPlan = { autoFixed: [], humanFixRequired: [] };
            if (args.autoFix) {
    ...
    defineTool({
        ...
        description: 'Генерирует подробный checklist ассетов для производства сцены. ' +
    ...
    defineTool({
        ...
        description: 'Генерирует полный план 360° рига для персонажа в Toon Boom Harmony. ' +
        ...
        handler: async (args) => {
            ...
            for (const view of views) {
                layerNaming[view] = allParts.map((part) => `${safeN}_${view}_${part}`);
            }
            // Harmony node structure
            const nodeStructure = {
                rootGroup: `Top/${safeN}`,
                subgroups: views.map((view) => ({
                    path: `Top/${safeN}/${safeN}_${view}`,
                    drawingNodes: allParts.map((part) => `${safeN}_${view}_${part}`),
...
${views.map((view) => `var ${view}Group = node.add("Top/${safeN}", "${safeN}_${view}", "GROUP", 0, 0, 0);`).join('\n')}
...
MessageLog.trace("Риг ${name} создан");`;
    ...
    defineTool({
        ...
        description: 'Собирает финальный пакет для клиента или ревью: ' +
