...
export const lipsyncTools = [
    ...
    defineTool({
        ...
        description: 'Генерирует lipsync_plan.json из текста диалогов и тайминга сцены. ' +
        ...
        handler: async (args) => {
            ...
            const dialogueLines = args.dialogues.map((d) => {
                ...
                for (const word of words) {
                    ...
                    for (let s = 0; s < syllables; s++) {
                        ...
                        let shape = 'A';
                        if ('pbm'.includes(char.toLowerCase()))
                            shape = 'B';
                        else if ('fv'.includes(char.toLowerCase()))
                            shape = 'F';
                        else if ('ln'.includes(char.toLowerCase()))
                            shape = 'G';
                        else if ('ae'.includes(char.toLowerCase()))
                            shape = 'A';
                        else if ('ou'.includes(char.toLowerCase()))
                            shape = 'E';
                        else if ('i'.includes(char.toLowerCase()))
                            shape = 'C';
                        else if ('r'.includes(char.toLowerCase()))
                            shape = 'D';
                        else
                            shape = 'A';
            ...
            const totalFrames = Math.max(...args.dialogues.map((d) => d.endFrame));
            const lipsyncPlan = {
                totalFrames,
                fps,
                engine,
            ...
            const totalPhonemes = dialogueLines.reduce((sum, d) => sum + d.phonemes.length, 0);
            return {
                status: 'success',
                engine,
                ...
                summary: {
                    ...
                    totalPhonemes,
                    characters: [...new Set(args.dialogues.map((d) => d.character))],
    ...
    defineTool({
        ...
        description: 'Применяет lipsync_plan.json к открытому проекту Harmony. ' +
