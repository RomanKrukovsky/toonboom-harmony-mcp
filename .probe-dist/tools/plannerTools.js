...
export const plannerTools = [
    defineTool({
        ...
        description: 'Validate a scene_plan.json object against the locked, versioned schema. ' +
    ...
    defineTool({
        ...
        description: 'Load and validate a scene_plan.json file from disk. Resolves against HARMONY_ALLOWED_ROOTS.',
        inputSchema: z.object({
    ...
    defineTool({
        ...
        description: 'Ingest shots for an episode from a Kitsu REST API and emit scene_plan.json objects. ' +
    ...
    defineTool({
        ...
        description: 'After Autopilot executes a scene plan, write the resulting status back to Kitsu tasks. ' +
    ...
    defineTool({
        ...
        description: 'Import a CSV or JSON shot list and produce scene_plan.json stubs for each shot. ' +
        ...
        handler: async (args) => {
            ...
            for (const p of plans) {
                try {
                    ...
                    errors.push({ sceneName: p.sceneName, error: e.message });
                }
            }
            return {
    ...
    defineTool({
        ...
        description: 'Generate a marketing-ready time-savings report: how long the assembled scene would take ' +
    ...
    defineTool({
        ...
        description: 'Создает единый пакет ревью для клиента: упаковывает рендеры, статический отчет аудита и метаданные в архив или папку.',
        inputSchema: z.object({
    ...
    defineTool({
        ...
        description: 'Генерирует валидный scene_plan.json из свободного текстового описания сцены. ' +
        ...
        inputSchema: z.object({
            prompt: z.string().describe('Текстовое описание сцены. Примеры: ' +
...
function parseCsv(raw) {
    ...
    return lines.slice(1).map(line => {
        ...
        if (obj.frames)
            obj.frames = parseInt(obj.frames, 10) || 0;
        if (obj.duration)
            obj.duration = parseFloat(obj.duration) || 0;
        return obj;
    });
}
