...
function walkFiles(root, current = root, collected = []) {
    let entries;
    try {
...
function loadLicenses(packageDir) {
...
function licenseForFile(file, licenses) {
...
function licensableFiles(packageDir) {
    return walkFiles(packageDir).filter(file => LICENSABLE_EXTENSIONS.has(file.extension) && path.basename(file.absolutePath) !== LICENSE_FILENAME);
}
function requirePackageDir(packageDir) {
...
export const legalTools = [
        ...
        handler: async (args) => {
            ...
            const assets = files.map(file => {
                ...
                return {
                    ...
                    digestMismatch,
                    status: (!verdict.allowed ? 'rejected' : digestMismatch ? 'digest_mismatch' : 'cleared'),
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            const governed = licensableFiles(packageDir).filter(file => licenseForFile(file, licenses)?.filePath === match.filePath);
            ...
            if (!contractPresent)
                blockers.push(`Файл договора отсутствует: ${license.contractPath}`);
            if (!digestOk)
                blockers.push('Ни один управляемый файл не совпадает с объявленным sha256.');
            return {
