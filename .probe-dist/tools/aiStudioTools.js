...
import { sceneUnderstandingSchema, SCENE_INTELLIGENCE_SCHEMA_VERSION } from '../schemas/sceneIntelligence.js';
...
import { sceneSnapshotPirSchema } from '../schemas/sceneSnapshotPir.js';
...
import { artistCorrectionSchema, datasetExportSchema, correctionTypeSchema } from '../schemas/artistCorrection.js';
...
const analyzeSceneSchema = z.object({
    script: z.string().min(1).describe('Текст сцены. Поддерживаются два формата: (1) многострочный сценарий с ' +
    ...
    characters: z.array(characterInputSchema).min(1).describe('Список участников сцены с именами. role/stance — опциональные подсказки; ' +
        'движок выведет их сам, если опущены.'),
    dialogue: z.array(dialogueLineInputSchema).optional().describe('Явный тайминг реплик — атрибуция и текст строк. Если опущено, движок попытается ' +
        'разобрать строки из script регуляркой.'),
    location: z.string().optional(),
    directorConstraints: z.array(z.string()).optional().describe('Режиссёрские ограничения, которые фиксируются в continuity_constraints и ' +
        'помечаются как locked. Например: "Не уводить камеру с Саши в первом бите".'),
    reportDir: z.string().optional().describe('Каталог под HARMONY_ALLOWED_ROOTS для записи HTML-отчёта. Если опущено — ' +
        'отчёт не сохраняется на диск, а только возвращается в результате.')
}).strict();
const generateVariantsSchema = z.object({
    sceneUnderstanding: sceneUnderstandingSchema.optional().describe('Ранее вычисленное SceneUnderstanding. Если опущено — script/characters/dialogue ' +
    ...
    ])).optional().describe('Явный список стратегий для вариантов. Если опущено — используются первые `count` ' +
...
export const aiStudioTools = [
    defineTool({
        ...
        description: 'Scene Understanding Engine (Iteration 1). Принимает текст сцены, реплики, персонажей ' +
        ...
        handler: async (args) => {
            ...
            if (args.reportDir) {
                ...
                reportPath = reportBuilder.buildToFile({ scene, variantSet: director.generateVariants(scene, Math.min(3, ALL_STRATEGIES.length)) }, path.join(dir, `${scene.sceneId}_scene_intelligence.html`));
            }
            return {
                status: 'success',
    ...
    defineTool({
        ...
        description: 'AI Director (Iteration 1). Принимает ранее вычисленное SceneUnderstanding (или набор ' +
        ...
        handler: async (args) => {
            ...
            if (!scene) {
                if (!args.script || !args.characters || args.characters.length === 0) {
                    throw new HarmonyError('INVALID_HARMONY_OBJECT', 'generate_director_variants: требуется либо sceneUnderstanding, либо (script + characters + dialogue).');
                }
            ...
            if (args.reportDir) {
                ...
                if (!fs.existsSync(dir))
                    fs.mkdirSync(dir, { recursive: true });
                reportPath = reportBuilder.buildToFile({ scene: sceneObj, variantSet }, path.join(dir, `${sceneObj.sceneId}_scene_intelligence.html`));
            }
            return {
                status: 'success',
                schemaVersion: SCENE_INTELLIGENCE_SCHEMA_VERSION,
    ...
    defineTool({
        ...
        handler: async (args) => {
            ...
            if (args.reportDir) {
                const dir = verifyPathAccess(args.reportDir);
                reportPath = voiceReportBuilder.buildToFile(set, path.join(dir, `${scene.sceneId}_${args.characterId}_performance.html`));
            }
    ...
    defineTool({
        ...
        handler: async (args) => ({ status: 'success', schemaVersion: VOICE_PERFORMANCE_SCHEMA_VERSION, performance: performanceGenerator.mix(args.acting, args.gestureTiming, args.finalPose) })
    }),
    defineTool({
