...
export function createRequest(
  ...
  method: string,
  params: Record<string, unknown>,
  options: { protocolVersion: string; correlationId?: string; idempotencyKey?: string } = {
...
export function classifyErrno(code: string | undefined): ErrorCode {
  switch (code) {
    case "ENOENT":
...
export function parseResponse(data: string): JsonRpcResponse {
  ...
  try {
    ...
    throw new WireProtocolError(
      `Invalid JSON from MOHO server: ${trimmed.slice(0, 200)}`,
  ...
  if (typeof obj.protocolVersion !== "string") {
    throw new WireProtocolError(
      "MOHO response missing protocolVersion",
      ErrorCodes.PROTOCOL_VERSION_MISSING,
    );
  }
