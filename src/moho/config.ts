 ...
 */
function validateProtocolVersions(): { current: string; min: string; max: string } {
  ...
  if (!semver.test(PROTOCOL_VERSION) || !semver.test(MIN_SUPPORTED) || !semver.test(MAX_SUPPORTED)) {
    throw new Error(
      `Invalid protocol version configuration: current=${PROTOCOL_VERSION}, min=${MIN_SUPPORTED}, max=${MAX_SUPPORTED}`,
    );
  }
 ...
 */
export async function ensureIpcDir(): Promise<string> {
