...
class Counter {
  public state: CounterState = freshState();
  ...
  inc(by = 1, durationMs?: number): void {
  ...
  private recordDuration(durationMs: number): void {
    ...
    if (this.state.observedSamples < MAX_COUNT) this.state.observedSamples += 1;
    if (buf.length < MAX_SAMPLES) {
      buf.push(durationMs);
      this.writeIndex = buf.length % MAX_SAMPLES;
      return;
    }
    if (this.writeIndex >= MAX_SAMPLES) this.writeIndex = 0;
...
export function quantile(values: number[], q: number): number | null {
  ...
  if (typeof q !== "number" || !Number.isFinite(q) || q < 0 || q > 1) return null;
...
export const SLO = {
  ...
  destructiveConfirmations: new Counter(
    "moho_mcp_destructive_confirmations_total",
    "Destructive ops that passed preview-hash validation",
  ),
} as const;

/** Render a value Prometheus can parse; never emit NaN/Infinity into the exposition. */
function promNumber(value: number): string {
  return Number.isFinite(value) ? String(value) : "0";
}

export function summarizeSlo(): string {
...
export function snapshotSlo(): CounterSnapshot[] {
