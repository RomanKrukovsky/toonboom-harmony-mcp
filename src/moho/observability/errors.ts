...
const REGISTRY: Record<ErrorCodeValue, ErrorCategoryMeta> = {
  ...
  [ErrorCode.PERMISSION_DENIED]: {
    ...
    retryable: false,
    recoverySuggestion:
      "Set the appropriate environment flag (e.g. MOHO_MCP_ENABLE_SCREENSHOTS) or grant the required permission in MohoMCP configuration.",
    category: "permission",
  },
  [ErrorCode.UNSUPPORTED_CAPABILITY]: {
    ...
    retryable: false,
    recoverySuggestion:
      "Verify Moho version on moho://upstream/version. Lua API may differ between versions.",
  ...
  [ErrorCode.ENTITY_NOT_FOUND]: {
    ...
    retryable: false,
    recoverySuggestion:
      "Re-query layer/bone ids (mcp tool returns are authoritative; never cache identifiers across operations).",
    category: "state",
  },
  ...
  [ErrorCode.UI_AUTOMATION_DISABLED]: {
    ...
    retryable: false,
    recoverySuggestion:
      "MOHO_MCP_ENABLE_UI_AUTOMATION is off by default. Enable explicitly after reviewing UI automation risks.",
    category: "permission",
  },
...
export const ErrorRegistry: Readonly<Record<ErrorCodeValue, ErrorCategoryMeta>> = Object.freeze(REGISTRY);

export class MohoBridgeError extends Error {
  public readonly payload: MohoErrorPayload;
  ...
  public toJSON(): MohoErrorPayload {
    return this.payload;
  }
}
...
export function buildError(
  ...
  const effectiveCode: ErrorCodeValue = known ? code : ErrorCode.INTERNAL_ERROR;
...
export function isRetryableMohoError(err: unknown): boolean {
  if (err instanceof MohoBridgeError) return err.payload.retryable;
  return false;
}

/** HTTP-like status for a code, for callers that map errors onto transports. */
export function httpLikeStatus(code: ErrorCodeValue): number {
