...
export class BridgeSpool {
  ...
  async probe(timeoutS = 2.0): Promise<{ up: boolean; detail: string; armed?: boolean }> {
    if (!fs.existsSync(this.spool)) {
      return { up: false, detail: `spool ${this.spool} does not exist` };
    }
    try {
      const r = await this.call<{ pong: boolean; armed: boolean }>('ping', {}, timeoutS);
  ...
  async call<T = any>(
    ...
    if (deadlineS <= 0) throw new Error('deadlineS must be positive');
  ...
  ping(timeoutS = 3.0) {
    return this.call<{ pong: boolean; armed: boolean }>('ping', {}, timeoutS);
  }

  capabilities(timeoutS = 10.0) {
    return this.call<{ version: string; protocol: number; probes: Record<string, boolean> }>(
      'capabilities', {}, timeoutS,
    );
  }

  status(timeoutS = 3.0) {
    return this.call<{ armed: boolean; busy: boolean; served: number; failed: number }>(
      'status', {}, timeoutS,
    );
  }
}

let shared: BridgeSpool | null = null;

/** Общий экземпляр: спул один на машину, плодить клиентов незачем. */
export function bridge(): BridgeSpool {
  if (!shared) shared = new BridgeSpool();
  return shared;
}
