...
export class AddressError extends Error {
  constructor(public code: string, message: string, public remedy?: string) {
    super(message);
...
function checkName(kind: string, value: string): void {
  ...
  if (!NAME_RE.test(value)) {
    throw new AddressError(
      'BAD_NAME',
      `${kind} ${JSON.stringify(value)} contains characters that break database addressing`,
      'Use letters, digits, underscore and dash only. Spaces and slashes silently ' +
 ...
 */
export function parseAddress(raw: string): SceneAddress {
  const s = raw.trim();
  if (!s) throw new AddressError('EMPTY', 'scene address is empty');

  if (!s.startsWith('db://')) {
    if (!s.endsWith('.xstage')) {
      throw new AddressError(
        'NOT_A_SCENE',
        `${JSON.stringify(s)} is neither a db:// address nor a .xstage file`,
        'Standalone scenes must point at the .xstage file itself.',
      );
    }
  ...
  if (parts.length !== 3) {
    throw new AddressError(
      'BAD_DB_ADDRESS',
      `expected db://ENVIRONMENT/JOB/SCENE, got ${JSON.stringify(s)}`,
    );
  }
...
export function formatAddress(a: SceneAddress): string {
  if (a.mode === 'standalone') return a.path;
  let s = `db://${a.environment}/${a.job}/${a.scene}`;
  if (a.version) s += `@${a.version}`;
  if (a.user) s += `#${a.user}`;
  return s;
}
 ...
 */
export function cliArgs(a: SceneAddress): string[] {
  if (a.mode === 'standalone') return [a.path];
  const out = ['-env', a.environment, '-job', a.job, '-scene', a.scene];
  if (a.version) out.push('-version', a.version);
  if (a.user) out.push('-user', a.user);
  return out;
}
 ...
 */
export function requireUnambiguousForWrite(a: SceneAddress): void {
  if (a.mode === 'standalone') return;
  if (!a.version) {
    throw new AddressError(
      'AMBIGUOUS_VERSION',
      `refusing to write to ${formatAddress(a)}: version is not specified`,
      'Resolve the version explicitly first (harmony.db.resolve), then write to that exact ' +
  ...
  if (!a.user) {
    throw new AddressError(
      'NO_USER',
      `refusing to write to ${formatAddress(a)}: database user is not specified`,
      'Harmony records who changed a scene. Anonymous writes make an audit trail useless.',
    );
  }
}

// ---------------------------------------------------------------------------
// Многопользовательность: блокировки и права
// ---------------------------------------------------------------------------
 ...
 */
export function decideWritable(
  state: LockState,
  owner: string | null,
  me: string | null,
): SceneLock {
  switch (state) {
    ...
    default:
      return {
        ...
        writable: false,
        reason:
          'lock state could not be determined; refusing to write. Harmony accepts mutations ' +
 ...
 */
export function writeProbePlan(a: SceneAddress): { script: string; note: string } {
  return {
    script:
      ...
      'ok;',
    note:
      `Write-probe for ${formatAddress(a)}: sets the resolution to its current value inside an ` +
