    ...
    child.on('close', (code) => {
      ...
      for (const line of out.trim().split('\n').reverse()) {
        ...
        if (t.startsWith('{') && t.endsWith('}')) {
          try { parsed = JSON.parse(t); break; } catch { /* не тот */ }
        }
      }
...
export function blenderStatus(): { present: boolean; path: string; detail: string } {
...
export function pipelineStatus(): { present: boolean; dir: string; missing: string[] } {
  const dir = clientDir();
