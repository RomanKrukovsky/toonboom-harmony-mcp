...
let registryPromise: Promise<RigTemplateRegistry> | null = null;
function getRegistry(): Promise<RigTemplateRegistry> {
  if (!registryPromise) {
 ...
 */
const MOUTH_CHART: Array<{ viseme: string; description: string; phonemes: string[] }> = [
...
const PART_KEYWORDS: Array<{ part: string; patterns: RegExp }> = [
...
function classifyLayer(name: string): string {
  for (const entry of PART_KEYWORDS) {
    if (entry.patterns.test(name)) return entry.part;
  }
  return 'unclassified';
}
 ...
 */
function readLayerNames(filePath: string): { format: string; layers: string[]; parseable: boolean; note?: string } {
...
export const riggingEngineTools = [
  defineTool({
    ...
    handler: async (args) => {
      ...
      const byPart = classified.reduce<Record<string, string[]>>((acc, item) => {
  ...
  defineTool({
    ...
    handler: async () => {
      ...
      for (const entry of entries) unique.set(entry.contentHash, entry);
  ...
  defineTool({
    ...
    handler: async (args) => {
      ...
      const required: string[] = entry.template.required_landmarks ?? [];
      ...
      let plan: any;
      try {
        plan = resolver.resolveBinding(args.characterId, args.pir as any, pirHash, entry);
      } catch (err: any) {
        return {
          ...createStandardExecutionResult({
            status: 'blocked',
      ...
      const unresolved: string[] = plan.unresolved_slots ?? plan.unresolved ?? [];
      const bindings: any[] = plan.bindings ?? [];
  ...
  defineTool({
    ...
    handler: async (args) => {
      ...
      for (const connection of (template.connections ?? []) as any[]) {
      ...
      for (const command of commands) {
        if (command.command === 'create_peg' || command.command === 'create_node') {
          ...
          for (const key of ['srcNodePath', 'destNodePath']) {
            ...
            if (!created.has(ref)) orderViolations.push(`${ref} используется до создания.`);
          }
        }
      }
  ...
  defineTool({
    ...
    handler: async (args) => {
      ...
      const specs: Record<string, { widget: string; range: [number, number]; drives: string }> = {
