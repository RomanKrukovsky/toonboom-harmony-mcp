...
export interface TypedTool<S extends z.ZodTypeAny = z.ZodTypeAny> {
  ...
  description: string;
  inputSchema: S;
  // The return value stays `any`: results are heterogeneous across 500+ tools and
  // the dispatcher serialises whatever comes back. The point of this helper is to
  // type the *input*, which is what silently drifted from the schema before.
 ...
 */
export function defineTools<T extends readonly TypedTool<any>[]>(tools: T): T {
  return tools;
}
