...
async function viaBridge<T>(
  ...
  deadlineS: number,
): Promise<{ ok: true; result: T; log: string[] } | { ok: false; error: any }> {
  try {
    const r = await bridge().call<T>(op, args, deadlineS);
...
export const bridgeTools = [
  defineTool({
    ...
    description:
      'Состояние файлового моста в Harmony: установлен ли, отвечает ли, вооружён ли. ' +
    ...
    handler: async (args) => {
      ...
      if (!probe.up) {
        return {
          ...
          remedy:
            'Install harmony/packages/mcp-bridge/ into Harmony user scripts ' +
            '(specialFolders.userScripts + "/packages/mcp-bridge"), then restart Harmony.',
        };
      }
      const st = await viaBridge<any>('status', {}, args.timeoutS ?? 3.0);
      return {
        up: true,
        spool: b.spool,
        armed: probe.armed,
  ...
  defineTool({
    ...
    description:
      'Карта API конкретной сборки Harmony: результаты ПРОБ отдельных функций. ' +
  ...
  defineTool({
    ...
    description:
      'Выполнить QtScript внутри работающего Harmony и получить JSON. ' +
    ...
    handler: async (a) =>
      viaBridge('eval', { script: a.script, args: a.args ?? {} }, a.deadlineS ?? 30.0),
  }),

  defineTool({
    name: 'harmony.bridge.render_frame',
    description:
      'Отрендерить один кадр и вернуть путь к PNG. Это «глаза» петли обратной связи: ' +
    ...
    handler: async (a) =>
      viaBridge(
        'render_frame',
        { frame: a.frame, width: a.width ?? 960, display: a.display,
          timeout_ms: Math.round((a.timeoutS ?? 180) * 1000) },
        a.timeoutS ?? 180,
      ),
  }),
  ...
  defineTool({
    ...
    description:
      'Отрендерить диапазон кадров с шагом — контактный лист. Нужен, чтобы видеть ДВИЖЕНИЕ, ' +
    ...
    handler: async (a) => {
      ...
      return viaBridge(
        'render_range',
        { from: a.from, to: a.to, stride: a.stride ?? 1, width: a.width ?? 480,
          timeout_ms: Math.round((a.timeoutS ?? 600) * 1000) },
        a.timeoutS ?? 600,
      );
    },
  }),
  ...
  defineTool({
    ...
    description:
      'Прочитать колонку экспозиции как таблицу: тайминг становится данными, а не мышкой по кадрам.',
    inputSchema: z.object({
    ...
    handler: async (a) =>
      viaBridge('xsheet_get',
        { column: a.column, columns: a.columns, from: a.from, to: a.to },
        a.timeoutS ?? 30),
  }),
  ...
  defineTool({
    ...
    description:
      'Записать ячейки экспозиции. МЕНЯЕТ СЦЕНУ. По умолчанию dryRun=true: ' +
    ...
    handler: async (a) => {
      ...
      return viaBridge('xsheet_set',
        { column: a.column, edits: a.edits, ripple: !!a.ripple },
        a.timeoutS ?? 60);
    },
  }),
  ...
  defineTool({
    ...
    description:
      'Прочитать функциональную кривую как безье-точки: изинг становится математикой, ' +
  ...
  defineTool({
    ...
    description:
      'Записать функциональную кривую. МЕНЯЕТ СЦЕНУ, dryRun=true по умолчанию. ' +
    ...
    handler: async (a) => {
      ...
      return viaBridge('curve_set',
        { column: a.column, keys: a.keys, replace: a.replace !== false },
        a.timeoutS ?? 60);
    },
  }),
  ...
  defineTool({
    ...
    description:
      'Статический анализ сцены: висячие ноды, мёртвые холды, плоские дуги, ' +
  ...
  defineTool({
    ...
    description:
      'Вооружить или разоружить мост. Разоружённый мост обслуживает только ping/status/' +
      ...
      if (a.armed && a.confirm !== 'yes-arm-the-bridge') {
        return {
          ...
          error: {
            ...
            message:
              'Arming the bridge enables arbitrary code execution inside Harmony with the ' +
              'user\'s privileges. Pass confirm:"yes-arm-the-bridge" to proceed.',
            remedy:
              'Prefer arming from inside Harmony (Windows > "MCP Bridge: Arm / Disarm") so a ' +
