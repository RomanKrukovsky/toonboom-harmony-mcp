...
type DraftOperation = Omit<HarmonySemanticOperation, 'opId'>;

function op(draft: DraftOperation): HarmonySemanticOperation {
  return { ...draft, opId: computeOperationId(draft) };
}

function connectionKey(c: SceneConnection): string {
...
function valuesEqual(a: unknown, b: unknown, epsilon: number): boolean {
  if (typeof a === 'number' && typeof b === 'number') return nearlyEqual(a, b, epsilon);
  return a === b;
}
...
function isTransformAttribute(attribute: string): boolean {
...
export class SemanticSceneDiffEngine {
   ...
   */
  public diff(before: HarmonySceneState, after: HarmonySceneState, options: SemanticDiffOptions = {}): HarmonyScenePatch {
    if (before.scenePathHash !== after.scenePathHash) {
      throw new HarmonyError(
        'INVALID_INPUT',
        'Refusing to diff two different scenes: scenePathHash differs between before and after.',
        { before: before.scenePathHash, after: after.scenePathHash }
      );
    }

    const epsilon = options.epsilon ?? DEFAULT_EPSILON;
    const warnings: string[] = [];
    const operations: HarmonySemanticOperation[] = [];
    ...
    operations.push(
      ...this.diffKeyframes(before.keyframes, after.keyframes, epsilon, options.detectKeyframeMoves !== false)
    );
    operations.push(
      ...this.diffExposures(before.exposures, after.exposures, options.detectExposureShifts !== false)
    );
    ...
    if (before.captureMode !== 'full' || after.captureMode !== 'full') {
      warnings.push(
        `patch computed from non-full capture (before=${before.captureMode}, after=${after.captureMode}); absent entities are not proof of deletion`
      );
    }
    ...
    const base = {
      ...
      kind: 'HarmonyScenePatch' as const,
    ...
    const patch: HarmonyScenePatch = { ...base, deterministicHash: computePatchHash(base) };
   ...
   */
  public invert(patch: HarmonyScenePatch, generatedAt?: string): HarmonyScenePatch {
    const inverse: HarmonySemanticOperation[] = [];
  ...
  private diffNodes(before: SceneNode[], after: SceneNode[]): HarmonySemanticOperation[] {
    const beforeMap = new Map(before.map(n => [n.path, n]));
    const afterMap = new Map(after.map(n => [n.path, n]));
    const ops: HarmonySemanticOperation[] = [];
    ...
    for (const [path, afterNode] of afterMap) {
      ...
      for (const property of ['positionX', 'positionY', 'enabled'] as const) {
  ...
  private diffConnections(before: SceneConnection[], after: SceneConnection[]): HarmonySemanticOperation[] {
  ...
  private diffAttributes(
    before: HarmonySceneState,
    after: HarmonySceneState,
    epsilon: number
  ): HarmonySemanticOperation[] {
    const cameraPaths = new Set(
      after.nodes.filter(n => n.type.toUpperCase() === 'CAMERA').map(n => n.path)
    );
    const nodeTypes = new Map(after.nodes.map(n => [n.path, n.type.toUpperCase()]));
    for (const node of before.nodes) {
      if (!nodeTypes.has(node.path)) nodeTypes.set(node.path, node.type.toUpperCase());
    }

    const key = (a: SceneNodeAttribute) => `${a.nodePath}|${a.attribute}`;
    ...
    const removedNodes = new Set(
      before.nodes.filter(n => !after.nodes.some(m => m.path === n.path)).map(n => n.path)
    );
    const addedNodes = new Set(
      after.nodes.filter(n => !before.nodes.some(m => m.path === n.path)).map(n => n.path)
    );

    const ops: HarmonySemanticOperation[] = [];

    const classify = (nodePath: string, attribute: string) => {
      if (cameraPaths.has(nodePath)) return 'change_camera_property' as const;
      const type = nodeTypes.get(nodePath) ?? '';
      if (PEG_NODE_TYPES.has(type) && isTransformAttribute(attribute)) return 'change_peg_transform' as const;
      return 'change_node_attribute' as const;
    };
    ...
    for (const [k, afterAttr] of afterMap) {
      ...
      if (!beforeAttr && addedNodes.has(afterAttr.nodePath)) continue;
    ...
    for (const [k, beforeAttr] of beforeMap) {
      ...
      if (removedNodes.has(beforeAttr.nodePath)) continue;
      ops.push(
        op({
  ...
  private diffKeyframes(
    ...
    for (const columnName of [...columns].sort()) {
      ...
      const added: SceneKeyframe[] = [];
      const removed: SceneKeyframe[] = [];
      ...
      for (const [frame, afterKey] of afterKeys) {
        ...
        if (
          beforeKey.interpolation !== afterKey.interpolation ||
      ...
      for (const [frame, beforeKey] of beforeKeys) {
        if (!afterKeys.has(frame)) removed.push(beforeKey);
      }
  ...
  private reconcileKeyframeAddRemove(
    ...
    if (detectMoves) {
      for (const removedKey of removed) {
        ...
        if (matchesInRemoved.length !== 1 || matchesInAdded.length !== 1) continue;

        const addedKey = matchesInAdded[0];
        if (consumedAdded.has(addedKey.frame) || consumedRemoved.has(removedKey.frame)) continue;
    ...
    for (const key of added) {
      if (consumedAdded.has(key.frame)) continue;
      ops.push(
        op({
          type: 'add_keyframe',
          origin: 'harmony_manual',
    ...
    for (const key of removed) {
      if (consumedRemoved.has(key.frame)) continue;
      ops.push(
        op({
  ...
  private diffCamera(before: HarmonySceneState, after: HarmonySceneState, epsilon: number): HarmonySemanticOperation[] {
    ...
    for (const property of [...new Set([...Object.keys(beforeProps), ...Object.keys(afterProps)])].sort()) {
      ...
      if (valuesEqual(beforeValue, afterValue, epsilon)) continue;
      ops.push(
        op({
          type: 'change_camera_property',
          origin: 'harmony_manual',
  ...
  private diffSceneSettings(before: HarmonySceneState, after: HarmonySceneState): HarmonySemanticOperation[] {
    const ops: HarmonySemanticOperation[] = [];
    const keys = Object.keys(after.sceneSettings) as Array<keyof HarmonySceneState['sceneSettings']>;
  ...
  private applyMcpClaims(
    operations: HarmonySemanticOperation[],
    claims: McpOperationClaim[]
  ): HarmonySemanticOperation[] {
    if (claims.length === 0) return operations;

    return operations.map(operation => {
      if (operation.origin !== 'harmony_manual') return operation;
      ...
      if (!targetName) return operation;

      const claim = claims.find(c => {
        if (!c.targets.includes(targetName)) return false;
        if (c.properties && c.properties.length > 0) {
          if (!operation.property || !c.properties.includes(operation.property)) return false;
        }
        if (c.frames && c.frames.length > 0) {
          if (operation.frame === undefined || !c.frames.includes(operation.frame)) return false;
        }
        return true;
      });
  ...
  private invertOperation(operation: HarmonySemanticOperation): HarmonySemanticOperation | undefined {
    const base = {
    ...
    switch (operation.type) {
      ...
      case 'shift_exposure': {
        const delta = (operation.after as { delta?: number } | undefined)?.delta;
  ...
  private summarize(operations: HarmonySemanticOperation[]): HarmonyScenePatch['summary'] {
    const nodesChanged = new Set<string>();
    const columnsChanged = new Set<string>();
    const framesTouched = new Set<number>();
    const operationCounts: Record<string, number> = {};

    for (const operation of operations) {
      operationCounts[operation.type] = (operationCounts[operation.type] ?? 0) + 1;
      if (operation.target.nodePath) nodesChanged.add(operation.target.nodePath);
      if (operation.target.columnName) columnsChanged.add(operation.target.columnName);
      ...
      if (operation.frame !== undefined) framesTouched.add(operation.frame);
