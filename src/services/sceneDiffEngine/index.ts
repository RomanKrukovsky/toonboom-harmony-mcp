import { SceneSnapshotPIR, SnapshotTransformKeyframe, SnapshotExposure } from '../../schemas/sceneSnapshotPir.js';
import { RetakeManifest, NodeDataDelta } from '../../schemas/retakeManifest.js';
import crypto from 'crypto';
import stringify from 'fast-json-stable-stringify';

export class SceneDiffEngine {
    private readonly epsilon = 0.00001; // For floating point comparisons

    public compare(v1: SceneSnapshotPIR, v2: SceneSnapshotPIR): RetakeManifest {
        const manifest: RetakeManifest = {
        ...
        for (const id of v2NodeIds) {
            if (!v1NodeIds.has(id)) manifest.nodes.added.push(id);
        }
        for (const id of v1NodeIds) {
            if (!v2NodeIds.has(id)) manifest.nodes.removed.push(id);
        }
        ...
        for (const c of v2.connections) {
            if (!v1ConnStrings.has(stringify(c))) manifest.connections.added.push(c);
        }
        for (const c of v1.connections) {
            if (!v2ConnStrings.has(stringify(c))) manifest.connections.removed.push(c);
        }
        ...
        for (const [nodeId, v2Data] of v2DataMap.entries()) {
            ...
            let transformKeysDelta: NodeDataDelta['transformKeys'];
            ...
            // Delta for exposures
            let exposuresDelta: NodeDataDelta['exposures'];
    ...
    private compareTransformKeys(v1: SnapshotTransformKeyframe[], v2: SnapshotTransformKeyframe[]): NonNullable<NodeDataDelta['transformKeys']> {
        const added: SnapshotTransformKeyframe[] = [];
        const modified: { original: SnapshotTransformKeyframe, updated: SnapshotTransformKeyframe }[] = [];
        const removed: SnapshotTransformKeyframe[] = [];
    ...
    private compareExposures(v1: SnapshotExposure[], v2: SnapshotExposure[]): NonNullable<NodeDataDelta['exposures']> {
        const added: SnapshotExposure[] = [];
        const modified: { original: SnapshotExposure, updated: SnapshotExposure }[] = [];
        const removed: SnapshotExposure[] = [];
    ...
    private keysEqual(k1: SnapshotTransformKeyframe, k2: SnapshotTransformKeyframe): boolean {
        return (
            Math.abs(k1.x - k2.x) < this.epsilon &&
