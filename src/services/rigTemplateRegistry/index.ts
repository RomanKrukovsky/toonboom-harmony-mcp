...
import { RigTemplate, rigTemplateSchema } from '../../schemas/rigTemplate.js';
...
export class RigTemplateRegistry {
  private templates: Map<string, RigTemplateEntry> = new Map();
  private readonly builtinPath: string;

  constructor(builtinPath: string = path.join(process.cwd(), 'templates', 'rig', 'builtin')) {
  ...
  public getTemplate(templateId: string, version?: string): RigTemplateEntry {
  ...
  public listTemplates(): RigTemplateEntry[] {
