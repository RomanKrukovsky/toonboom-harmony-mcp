...
import { HarmonyCommandPlanV4, HARMONY_COMMAND_PLAN_V4 } from '../../schemas/harmonyCommandPlanV4.js';
import crypto from 'crypto';
import stringify from 'fast-json-stable-stringify';

export class HarmonyCommandBuilder {
  public buildPlan(
    pir: CharacterTopologyPIR,
    bindingPlan: RigBindingPlanV1,
    templateEntry: RigTemplateEntry
    ...
    for (const binding of bindingPlan.bindings) {
      if (binding.resolution === 'DIRECT' && binding.pir_landmark) {
        ...
        if (landmark) {
          ...
          let targetNodeId = '';
          if (binding.template_slot === 'head_top') targetNodeId = 'NODE_HEAD_PEG';
          else if (binding.template_slot.startsWith('shoulder_left')) targetNodeId = 'NODE_LEFT_ARM_PEG';
          else if (binding.template_slot.startsWith('shoulder_right')) targetNodeId = 'NODE_RIGHT_ARM_PEG';
          else if (binding.template_slot === 'neck') targetNodeId = 'NODE_BODY_PEG';
  ...
  public buildAnimationPlan(retargetingPlan: RetargetingPlan): HarmonyCommandPlanV4 {
    const commands: HarmonyCommandPlanV4['commands'] = [];
  ...
  public buildLipSyncPlan(exposures: {nodeId: string; startFrame: number; endFrame: number; drawingName: string}[], sourceHash: string): HarmonyCommandPlanV4 {
    const commands: HarmonyCommandPlanV4['commands'] = [];
  ...
  public buildInbetweenPlan(pir: import('../../schemas/inbetweenPir.js').InbetweenPIR, targetNodeId: string): HarmonyCommandPlanV4 {
    const commands: HarmonyCommandPlanV4['commands'] = [];
  ...
  public buildRetakePatchPlan(retakeManifest: RetakeManifest): HarmonyCommandPlanV4 {
    const commands: HarmonyCommandPlanV4['commands'] = [];
