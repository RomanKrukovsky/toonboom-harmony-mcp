...
export class OpenRouterClient {
  ...
  constructor(apiKey?: string, defaultModel?: string) {
    this.apiKey = apiKey || config.backends.apiKeys.openrouter || process.env.OPENROUTER_API_KEY || '';
  ...
  public async complete(request: OpenRouterRequest): Promise<OpenRouterResponse> {
    ...
    try {
      ...
      console.error('OpenRouterClient completion error:', error);
      throw new Error(`OpenRouter completion failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
