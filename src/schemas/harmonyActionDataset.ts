...
export const HARMONY_ACTION_SCHEMA_VERSION = '1.0.0' as const;
export const HARMONY_ACTION_RECORDER_VERSION = 'harmony-action-recorder/1.0.0' as const;
 ...
 */
function byKeys<T>(...keys: Array<(item: T) => string | number>) {
...
export const canonicalSort = {
  nodes: (items: SceneNode[]) => [...items].sort(byKeys<SceneNode>(n => n.path)),
  connections: (items: SceneConnection[]) =>
    [...items].sort(byKeys<SceneConnection>(c => c.toNode, c => c.toPort, c => c.fromNode, c => c.fromPort)),
  nodeAttributes: (items: SceneNodeAttribute[]) =>
    [...items].sort(byKeys<SceneNodeAttribute>(a => a.nodePath, a => a.attribute)),
  columns: (items: SceneColumn[]) => [...items].sort(byKeys<SceneColumn>(c => c.name)),
  keyframes: (items: SceneKeyframe[]) =>
    [...items].sort(byKeys<SceneKeyframe>(k => k.columnName, k => k.frame)),
  exposures: (items: SceneExposure[]) =>
    [...items].sort(byKeys<SceneExposure>(e => e.nodePath, e => e.frame)),
  operations: (items: HarmonySemanticOperation[]) =>
    [...items].sort(
      byKeys<HarmonySemanticOperation>(
        o => o.type,
        o => o.target.nodePath ?? '',
        o => o.target.columnName ?? '',
        o => (o.target.connection ? `${o.target.connection.toNode}:${o.target.connection.toPort}` : ''),
        o => o.property ?? '',
        o => o.frame ?? -1,
        o => o.opId
      )
...
export function canonicalHash(value: unknown): string {
  return crypto.createHash('sha256').update(stringify(value as any)).digest('hex');
}

/** SHA-256 of a canonicalized filesystem path; the raw path never enters an artifact. */
export function hashScenePath(scenePath: string): string {
 ...
 */
export function computeSceneStateHash(
  state: Omit<HarmonySceneState, 'deterministicHash'> | HarmonySceneState
): string {
...
export function computePatchHash(
  patch: Omit<HarmonyScenePatch, 'deterministicHash'> | HarmonyScenePatch
): string {
  return canonicalHash({
...
export function computeOperationId(op: Omit<HarmonySemanticOperation, 'opId'>): string {
  return canonicalHash({
    type: op.type,
    target: op.target,
